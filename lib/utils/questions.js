...
// ── Answer-∈-choices matcher (single source of truth) ────────────────────────
// Find the index of the correct choice. For string-only choices this is the
// lenient text-match logic (strip label, trim, then math-equivalence via
// mathToCanvasHTML). For graph-bearing choice arrays the answer is expected to
// be a single letter (A/B/C/D) and we resolve it directly. Returns -1 when no
// choice matches the answer.
//
// This is the SAME matcher the QTI builders/export gate use (qti.js imports and
// re-exports it), and it is what validateQuestion's answer-membership check
// runs on — so the badge, the validator, and the export gate never disagree.
// See docs/exam_pipeline_design.md §4 (Inconsistency 1).
...
// True when the question's correct answer can be resolved to one of its choices
// using the lenient matcher. Non-MC types and choice-less questions are out of
// scope (return true; handled elsewhere).
...
// Resolve the AI-validation status to display for a question. A question's own
// validationStatus wins. Otherwise — variants carry a uid() id that isn't in
// the bank, so their own status is null on reload — fall back to the bank
// source row by originalId and mark it inherited:true. Returns a stable shape
// { status, issues, validatedAt, inherited }. Single source of truth shared by
// QuestionCard (badge) and ExportScreen (export-side badge + soft-confirm gate)
// so both screens agree. See docs/exam_pipeline_design.md §4 (Inconsistency 2/3).
export function resolveInheritedStatus(q, bankArr) {
  if (!q) return { status: null, issues: [], validatedAt: null, inherited: false };
...
export function validateQuestion(q) {
  ...
  // Branched MCQ keeps its prompt text in `stem` and per-part text in
  // parts[].question — the top-level `question` field is unused for that type.
  if (q.type === "Branched MCQ") {
    if (!q.stem || !String(q.stem).trim()) issues.push("Stem text is empty");
  } else if (!qText || !String(qText).trim()) {
    issues.push("Question text is empty");
  }

  if (q.type !== "Free Response" && q.type !== "Branched Free Response" && q.type !== "Branched MCQ") {
  ...
  if (q.type === "Multiple Choice" || q.type === "True/False") {
    ...
    if (q.answer && choices.length && !hasGraphChoices) {
      // String-only choices — answer must match one of them. Use the lenient
      // shared matcher (not exact includes) so the badge agrees with the QTI
      // export gate: a leniently-matching answer (label/whitespace/math-equiv)
      // exports fine and must not be flagged here. See exam_pipeline_design §4.
  ...
  if (q.type === "Branched MCQ") {
    ...
    if (parts.length < 2) {
      issues.push("Branched MCQ must have at least 2 parts");
    } else {
      parts.forEach((p, i) => {
        const lbl = `Part (${String.fromCharCode(97 + i)})`;
        if (!p || typeof p !== "object") {
          issues.push(`${lbl}: missing object`);
          return;
        }
        if (!p.question || !String(p.question).trim()) issues.push(`${lbl}: question text is empty`);
        if (!Array.isArray(p.choices) || p.choices.length < 2) {
          issues.push(`${lbl}: must have at least 2 choices`);
        } else if (p.answer != null && !p.choices.map(c => stripChoiceLabel(c)).includes(stripChoiceLabel(p.answer))) {
          issues.push(`${lbl}: correct answer does not match any choice`);
        }
        if (!p.answer && p.answer !== 0) issues.push(`${lbl}: answer is empty`);
      });
    }
  }
  ...
  if (q.hasGraph && q.graphConfig) {
    ...
    if (gc.type === "overlay") {
      if (!Array.isArray(gc.fns) || gc.fns.length === 0) {
        issues.push("Overlay graph missing fns array");
      } else {
        gc.fns.forEach((entry, i) => {
          if (!entry || typeof entry !== "object" || !entry.fn) {
            issues.push(`Overlay fns[${i}] missing fn`);
          }
        });
      }
    }
