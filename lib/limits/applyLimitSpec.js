...
function askLabel(quantity, at) {
  switch (quantity) {
    ...
    case "classifyDiscontinuity": return `the behavior of f at x = ${at}`;
...
function askValueString(spec, ask) {
  ...
  switch (ask.quantity) {
    ...
    case "classifyDiscontinuity":
      if (d.continuous) return "continuous";
      // kind "none" + not continuous = a domain endpoint (one side undefined).
      if (d.discontinuityKind === "none") return "defined on one side only";
      return d.discontinuityKind; // removable | jump | infinite
...
export function applyLimitSpec(q) {
  ...
  if (asks.some((a) => GLOBAL_QUANTITIES.has(a.quantity))) {
    ...
    out.explanation = composeContinuityStatement(records, "with_reasons", opts);
