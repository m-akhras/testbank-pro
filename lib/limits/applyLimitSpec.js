...
import { composeReasonedChoices, REASONED_QUANTITIES } from "./composeReasonedChoices.js";
...
export function applyLimitSpec(q) {
  ...
  if (isMC) {
    // SPEC-BACKED noGraph piecewise (single supported ask): the SYSTEM composes the
    // value+reason choices from derived facts and REPLACES the model's placeholders.
    // The model never writes a reason (its choices don't test understanding). Graph
    // styles and multi-ask questions keep the bare-value matching below.
    const single = asks.length === 1 ? asks[0] : null;
    if (q.noGraph && single && REASONED_QUANTITIES.has(single.quantity)) {
      const { choices, answer } = composeReasonedChoices(q.limitSpec, single);
      out.choices = choices;
      out.answer = answer;
      return out;
    }
