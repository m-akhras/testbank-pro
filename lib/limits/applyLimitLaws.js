...
import { applyLimitAtInfinitySpec } from "./applyLimitAtInfinity.js";
...
// (limitSpecF/limitSpecG/lawAsks) routes to applyLimitLaws; a §2.6 question
// (limitAtInfinitySpec) routes to applyLimitAtInfinitySpec; a §2.2/§2.5 question
// (limitSpec) routes to applyLimitSpec exactly as today; a plain question is a
// no-op. Carrying MORE THAN ONE of these shapes is ambiguous → hard-fail (never
// guess).
//
...
export function applyLimitDerivation(q, opts = {}) {
  ...
  const hasAtInf = !!q.limitAtInfinitySpec;
  if (hasPair && hasSingle) {
    throw new Error(
      "§2.3 ambiguous question: carries both a single limitSpec and a limitSpecF/limitSpecG pair"
    );
  }
  if (hasAtInf && (hasPair || hasSingle)) {
    throw new Error(
      "§2.6 ambiguous question: limitAtInfinitySpec carried with a §2.2/§2.3 spec"
    );
  }
  const result = hasAtInf
    ? applyLimitAtInfinitySpec(q)
    : hasPair
    ? applyLimitLaws(q)
    : applyLimitSpec(q);

  if (opts.requireSpecForGraph && !hasPair && !hasSingle && !hasAtInf) {
