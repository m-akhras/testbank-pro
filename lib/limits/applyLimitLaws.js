...
export function applyLimitLaws(q) {
  ...
  if (isMC) {
    // INJECTION model (mirrors §2.5 GLOBAL asks): the system composes the correct
    // combined text; the model's choices are DISTRACTORS. We do NOT phrase-match
    // the model's answer (correct math worded differently was being wrongly
    // rejected). Reuse a model choice if it already equals the system text;
    // otherwise replace the LAST distractor. Hard-fail only for structural issues.
