...
//
// graph_at_infinity is RATIONAL-ONLY in v1: a graph block is only legal on a
// rational derive-spec, and even then the curve must be provably pole-free. The
// two layers enforcing this:
//   - a rational-only contract restriction (root/special are symbolic-only); and
//   - a magnitude-based pole check that catches an EVEN-order pole (e.g. den = 3x^2)
//     that lands between samples with no sign change — the case the finiteness /
//     sign-change checks alone can miss.
//
...
const POLE_CAP = 1e6;
// Fine-sample resolution shared by the in-window pole check and the magnitude check.
const FINE_N = 600;
// A pole-free deg(num)<=deg(den) rational is bounded near its HA; a pole makes |f|
// blow up. We reject when the in-window peak exceeds this multiple of the HA scale —
// 25x headroom clears legitimate bumps while catching effective vertical asymptotes.
const SPIKE_HEADROOM = 25;
...
// Max |fn| over the SAME fine sample set used by hasInWindowPole. Non-finite samples
// are already rejected upstream, so they are skipped here rather than re-thrown.
function maxAbsOnWindow(fn, lo, hi) {
  let maxAbs = 0;
  for (let i = 0; i <= FINE_N; i++) {
    const x = lo + ((hi - lo) * i) / FINE_N;
    const y = fn(x);
...
export function applyLimitAtInfinitySpec(q) {
  ...
  // LAYER 1(a) — RATIONAL-ONLY GRAPHS. A graph block is legal only on a rational
  // derive-spec. root_over_linear always has a pole at x=-e/d, so it can never be
  // graphed pole-free; special forms are symbolic-only. Both are graph-illegal in v1.
  if (derive.kind !== "rational") {
    throw new Error(
      "§2.6 graph_at_infinity v1 supports rational curves only — root/special are symbolic-only"
    );
  }
  ...
  // LAYER 1(b) — MAGNITUDE-BASED POLE CHECK. An even-order pole (e.g. den = 3x^2)
  // can land between samples with no sign change, slipping past the finiteness /
  // sign-change checks above. A pole-free deg(num)<=deg(den) rational is bounded near
  // its HA, so an in-window peak far above the HA scale is an effective vertical
  // asymptote — reject it.
  const maxAbs = maxAbsOnWindow(fn, lo, hi);
  const haVals = result.horizontalAsymptotes.map((v) => Math.abs(v.num / v.den));
  const haScale = 1 + Math.max(0, ...haVals);
  if (maxAbs > SPIKE_HEADROOM * haScale) {
    throw new Error(
      `§2.6 graph fn spikes to ~${maxAbs} (effective vertical asymptote) — ` +
      "graph_at_infinity requires a pole-free curve"
    );
  }
