...
export function compileToGraphConfig(spec) {
  ...
  // Pieces, with ARROW flags on any end that DIVERGES (the curve runs off to
  // ±infinity — a VA-bordering end, where evalAt is non-finite). The renderer
  // already turns extendsLeft/extendsRight into the fn-arrow marker, so this is a
  // pure config enrichment (no renderer change). A finite end never gets an arrow
  // (it gets an endpoint dot below instead).
  const pieces = segments.map((s) => {
    const piece = { fn: s.fn, domain: [s.from, s.to] };
    if (evalAt(s.fn, s.from) === null) piece.extendsLeft = true;
    if (evalAt(s.fn, s.to) === null) piece.extendsRight = true;
    return piece;
  });
  ...
  // Endpoint dots: a CLOSED (included) finite segment end gets a FILLED dot,
  // UNLESS (a) a hole/point is already declared there, or (b) it's a seamless
  // join — another segment meets it at the same (x, y) (a continuous interior
  // join needs no marker). Diverging ends (evalAt null) are arrows, handled above.
  const isJoin = (self, x, y) =>
    segments.some((t) => {
      if (t === self) return false;
      if (!near(t.from, x) && !near(t.to, x)) return false;
      const ty = evalAt(t.fn, x);
      return ty !== null && near(ty, y);
    });
  segments.forEach((s) => {
    if (!s.openLeft) {
      const y = evalAt(s.fn, s.from);
      if (y !== null && !isJoin(s, s.from, y) && !hasCoord(holes, s.from, y) && !hasCoord(points, s.from, y)) {
        points.push([s.from, y]);
      }
    }
    if (!s.openRight) {
      const y = evalAt(s.fn, s.to);
      if (y !== null && !isJoin(s, s.to, y) && !hasCoord(holes, s.to, y) && !hasCoord(points, s.to, y)) {
        points.push([s.to, y]);
      }
    }
  });
