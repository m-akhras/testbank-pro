...
// renderer draws as two side-by-side piecewise panels (labeled f and g) in a
// single SVG. Each sub-config is produced by the SAME compileToGraphConfig (so it
// is byte-identical to a standalone §2.2/§2.5 graph and carries its own
// xDomain/xTickStep), and each is validated by it (phantom-VA / interior-pole
// guards) — a bad spec throws, failing closed on the same path §2.2 uses. Because
// the question still carries ONE graphConfig, QTI/Word export is unchanged: the
// renderer emits one combined PNG.
export function compileToFunctionPairConfig(specF, specG) {
  return {
    type: "functionPair",
    f: compileToGraphConfig(specF),
    g: compileToGraphConfig(specG),
  };
}
