...
/**
 * Canonical continuity prose — ALWAYS kind-free (bare x-values only). Modes:
 ...
 *
 * Discontinuity KINDS are deliberately NOT appended to the composed answer
 * statement (classification was removed in ee5c84d, and the parenthesized "(kind)"
 * format visually flagged the correct MC choice). Explanations may still describe
 * the behavior in prose — that's pedagogy — but the answer statement stays bare.
 * Any mode value other than "continuous_except" yields the "discontinuous at"
 * form, so a stray "with_reasons" can never re-introduce kinds.
 */
export function composeContinuityStatement(items, mode = "discontinuous_at", opts = {}) {
  ...
  const list = joinList(norm.map((it) => `x = ${it.x}`)); // bare — never kind-annotated
  if (mode === "continuous_except") return `${f} is continuous except at ${list}`;
  return `${f} is discontinuous at ${list}`;
}
