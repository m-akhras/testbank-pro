...
FUNCTION LETTERS (MANDATORY): the two functions MUST be called f and g. Write "f", "g", "f(x)", "g(x)" everywhere in the question text — NEVER h, y, or other letters. The graphs are labeled f and g, so any other letter contradicts the figure.
...
- READABLE Y-RANGE: every segment's values over its DISPLAYED interval (and every declared hole/point) must stay within roughly [-8, 8]. Choose slow-growing expressions or narrow each segment's domain — a fast-growing exponential over a wide interval (e.g. 2^x on [0,5] reaching 32) makes the shared graph unreadable and is REJECTED when its feature values fall outside the readable span. Both graphs share ONE y-axis, so keep f and g comparable in scale.
...
Do NOT emit a limitSpec for this style. Ask "Evaluate the limit, if it exists" for ONE concrete expression — EXACTLY ONE limit per question. Compute the answer yourself and show the full method.
...
Do NOT emit a limitSpec for this style. State given limit values at a point — lim f = a, lim g = b, lim h = c with EXACTLY ONE of them equal to 0 — then ask ONE combination to evaluate, e.g. "evaluate lim x→2 [g(x)*h(x)]/f(x)". Compute the answer yourself using the Limit Laws. EXACTLY ONE limit per question (a single combination — never a multi-part a/b/c list).
Method: apply sum/constant-multiple/product/quotient/power laws; the quotient law requires the denominator's limit != 0 (state this when the answer is DNE).
BATCH-LEVEL VARIETY (across the generated set, NOT within one question — several questions MAY reuse the same given-limits setup with different combinations, Stewart-style): include one question whose answer is DNE — a QUOTIENT with the zero-limit function in the denominator over a nonzero-limit numerator (e.g. lim g/h, lim h = 0); one question where the zero-limit function multiplies harmlessly to 0 (e.g. lim g*h/f = 0, valid because lim f != 0); and one power/root question (e.g. lim [g]^3, lim sqrt(f) — sqrt needs the limit >= 0).
Multiple Choice: choices are BARE SCALARS — a number like "-6", or "0", or "does not exist (DNE)". Author 4 plausible WRONG bare-scalar distractors; never pack several parts into one choice.
DISTRACTORS: treat 0/nonzero and nonzero/0 the same; compute b*0 as something other than 0; forget that sqrt of a negative limit is not real.
`;

const SQUEEZE_THEOREM_BLOCK = `
§2.3 SQUEEZE-THEOREM STYLE — SYMBOLIC, COMPUTE AND STATE THE ANSWER:
Do NOT emit a limitSpec for this style. Ask "Use the Squeeze Theorem to show that [limit] = 0" for a product of a factor that → 0 and a genuinely BOUNDED factor (sin or cos of anything, or e^(sin(...))). EXACTLY ONE limit per question. Compute and prove it.
...
§2.3 PIECEWISE-EVAL STYLE — SPEC-BACKED, DO NOT STATE THE ANSWER (no graph):
DISPLAY a 2-4 branch piecewise function f ALGEBRAICALLY in the question TEXT
(mini-syntax), e.g. f(x) = { x + 1 if x < 2 ; x^2 if x >= 2 }. There is NO graph for
this style: set "noGraph": true on the question.
You MUST ALSO emit a "limitSpec" encoding the SAME branches, plus an "asks" array with
EXACTLY ONE ask (a single one-sided limit, a single two-sided limit, OR a single
function value at a boundary — never a multi-part i/ii/iii list). The system DERIVES
the answer from the spec; THE SPEC IS THE ANSWER'S SOURCE OF TRUTH — do not state it
yourself, and do not write an explanation that contradicts it.
ENCODE the displayed branches (see the LIMIT SPEC contract below for the schema): one
"segments" entry per branch (from/to + openLeft/openRight matching strict <,> vs
inclusive <=,>=); a value defined SEPARATELY at a point → points[] { "x", "y" }; a
removable hole → holes[]. Each segment continuous on its own interval.
CRITICAL: the spec MUST encode the same function as the displayed formulas.
BATCH-LEVEL VARIETY (across the set, NOT within one question — questions MAY reuse the
same function with different asks): cover one-sided limit asks; at least one two-sided
limit that is DNE at a jump (one-sided limits differ); and at least one point where the
two-sided limit EXISTS but differs from f(a).
ASKS: leftLimit / rightLimit / limit / fValue at the boundary points.
MULTIPLE CHOICE: emit exactly 4 PLACEHOLDER choices (any strings — for schema validity
only). The system COMPOSES all four choices as "value, because reason" entirely from the
derived facts (the correct choice AND the three distractors) and REPLACES yours; q.answer
is set to the composed correct choice. DO NOT write the values or the reasons yourself —
your choices are discarded.
`;
...
export const calc1_2_3_template = {
  ...
  section_specific_rules: {
    ...
    conditional_quality_blocks: [
      ...
      {
        // ── ROUTING: piecewise_eval → SPEC-BACKED (no graph); system derives ──
        // Append the contract (the MIXED block already carries it via the
        // graph_limit_laws f/g blocks, so it isn't embedded in PIECEWISE_EVAL_BLOCK).
