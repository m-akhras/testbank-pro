...
§2.3 GRAPH-LIMIT-LAWS STYLE — EMIT TWO LIMIT SPECS + lawAsks, DO NOT STATE THE ANSWERS:
For this style you MUST output TWO declared LimitSpecs — "limitSpecF" (the graph of f) and "limitSpecG" (the graph of g) — plus a "lawAsks" array. You MUST NOT compute or assert any combined limit yourself: the system DERIVES every answer from the two specs and the law and OVERWRITES "answer"/"explanation" (leave them as best-effort placeholders). A malformed spec is REJECTED.

QUESTION TEXT (Stewart style): "The graphs of f and g are given. Use them to evaluate each limit, if it exists. If the limit does not exist, explain why." Then list the parts (a), (b), (c), ... in the SAME ORDER as your lawAsks.

lawAsks: [ { "law": "...", "at": <integer>, "params": { ... } } ]
- "sum"           — lim [f + g] as x→at
...
RULES:
- Use 3-6 asks per question, all at INTEGER points where BOTH f and g are defined.
- Place asks at the graphs' interesting features (jumps, holes, vertical asymptotes) so the answers are instructive, not all trivial substitutions.
- REQUIRED ask mix in EVERY question:
  (1) at least one "quotient" at a point where g's two-sided limit is 0 (the system will derive "does not exist (DNE)");
  (2) at least one "sum" or "difference" at a point where one operand's two-sided limit does not exist (e.g. a jump in f or g), so the combined limit is DNE;
  (3) at least one of "xPolyTimesF", "power", or "root".
...
- Multiple Choice: the SYSTEM composes the correct answer text from the derived values and INJECTS it as the answer key — you CANNOT set the correct answer. Author ALL choices as plausible WRONG distractors, written in the SAME compound sentence format the system uses (one labeled part per ask, joined by "; "), for example: "the limit of (f + g) as x approaches 3 = 4; the limit of f/g as x approaches 1 = does not exist (DNE); the limit of (x^2)*f as x approaches 2 = 8". Provide at least 2 distractor choices; the system replaces one of them with the correct composed text and marks it. Do NOT try to match the system's exact wording — only the math in your distractors needs to be plausibly wrong.
...
export const calc1_2_3_template = {
  ...
  section_specific_rules: {
    ...
    reference_examples: [
      ...
      {
        stem: "The graphs of f and g are given. Use them to evaluate each limit, if it exists. If the limit does not exist, explain why. (a) lim x→2 [f(x) + g(x)]  (b) lim x→0 [f(x) - g(x)]  (c) lim x→-1 [f(x) g(x)]  (d) lim x→3 f(x)/g(x)  (e) lim x→2 [x^2 f(x)]",
        answer: "Each part is read off the two graphs and combined by a limit law; parts where an operand's two-sided limit does not exist (a jump), or where the denominator's limit is 0, do not exist (DNE).",
        explanation: "Apply the Limit Laws to the values read from the graphs: (a) sum = lim f + lim g; (b) difference; (c) product; (d) quotient — DNE when lim g = 0 (nonzero numerator over 0); (e) lim[x^2 f] = (2^2)(lim x→2 f) since x^2 is continuous. A part is DNE whenever a needed one-sided pair disagrees (jump) so the operand's two-sided limit fails. (The textbook also has a part combining a function VALUE with a limit, which this generator does not produce.)"
      },
