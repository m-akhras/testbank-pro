...
Do NOT emit a limitSpec for this style — the piecewise function is given ALGEBRAICALLY (by its branch formulas and conditions), with no graph. Build a 3-4 piece piecewise function and ask ONE thing about it: a single one-sided limit, a single two-sided limit, OR a single function value at a boundary. EXACTLY ONE ask per question (never a multi-part i/ii/iii list).
Method: substitute the matching adjacent branch into a one-sided limit; the two-sided limit exists iff the one-sided limits agree; a function value is read from the branch (or special case) covering that point.
BATCH-LEVEL VARIETY (across the generated set, NOT within one question — several questions MAY share the same piecewise function with different asks): cover one-sided limit asks; at least one two-sided limit that is DNE at a jump (one-sided limits differ); and at least one point where the two-sided limit EXISTS but differs from f(a) (e.g. f(1) defined as 3 while lim x→1 f = 1).
Notation: write one-sided limits as "lim x→a^- f(x)" / "lim x→a^+ f(x)"; write "does not exist (DNE)" when they differ.
Multiple Choice: choices are BARE SCALARS — a number, or "does not exist (DNE)". Author 4 plausible WRONG bare-scalar distractors; never pack several parts into one choice.
DISTRACTORS: use the function value where a limit is asked (or vice versa); evaluate the wrong branch for a one-sided limit; claim the two-sided limit exists when the one-sided limits differ.
`;
