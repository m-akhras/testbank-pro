...
import { calc1_2_3_template } from "./calc1_2_3.js";
import { calc1_2_5_template } from "./calc1_2_5.js";
import { calc1_2_6_template } from "./calc1_2_6.js";
...
export const TEMPLATE_REGISTRY = [
  ...
  { course: "Calculus 1", sectionPrefix: "2.3 ", template: calc1_2_3_template },
  { course: "Calculus 1", sectionPrefix: "2.5 ", template: calc1_2_5_template },
  { course: "Calculus 1", sectionPrefix: "2.6 ", template: calc1_2_6_template },
];
...
// The limit-engine templates: the ONLY sections where a graph question must be
// backed by a derived spec (limitSpec, or limitSpecF+limitSpecG+lawAsks). The
// dispatch guard (applyLimitDerivation) uses this to reject a graph-without-spec
// in these sections WITHOUT affecting QM / Calc-3 / other sections that
// legitimately carry a hand-authored graphConfig. Explicit id allowlist (not a
// heuristic) so a future template can't accidentally opt into the guard.
const LIMIT_TEMPLATE_IDS = new Set(["calc1_2_2", "calc1_2_3", "calc1_2_5", "calc1_2_6"]);

/**
 * True when (course, section) resolves to one of the limit-engine templates.
 * @param {string} course
 * @param {string} section
 * @returns {boolean}
 */
export function isLimitTemplateSection(course, section) {
  const t = findTemplate(course, section);
  return !!(t && LIMIT_TEMPLATE_IDS.has(t.id));
}
