...
const REFERENCE_TEMPLATE_CALC_1_1 = String.raw`export const calc1_1_1_template = {
  ...
  fields: [
    ...
    },
    {
      ...
      default: [
        { value: "evaluate", count: 2, difficulty: "medium" },
        { value: "find_domain", count: 2, difficulty: "medium" }
      ],
      minSelections: 1,
      options: [
...
\`
    ...
    reference_examples: [
      {
        stem: "Let f(x) = 3x^2 - x + 2. Find f(-2).",
        choices: ["16", "12", "8", "-12"],
        answer: "16",
        explanation: "Substitute x = -2: f(-2) = 3(-2)^2 - (-2) + 2 = 12 + 2 + 2 = 16."
      },
      {
        stem: "What is the domain of g(x) = sqrt(x - 4)?",
        choices: ["[4, infinity)", "(4, infinity)", "(-infinity, 4]", "all real numbers"],
        answer: "[4, infinity)",
        explanation: "The radicand must be non-negative: x - 4 >= 0, so x >= 4. Domain is [4, infinity)."
      }
    ]
  }
};`;
...
3. The "type" of each field (number, single_select, multi_select, multi_select_with_counts, free_text).
4. The presence of minSelections: 1 on every multi_select / multi_select_with_counts field.
...
E. **student_tasks.options** — the cognitive actions students perform. For §1.3 transformations, tasks include: identify_transformation, apply_transformation, match_graph, compute_composition. For §5.2, tasks include: set_up_Riemann_sum, evaluate_definite, apply_FTC, interpret_area.
   ...
   IMPORTANT: The student_tasks field should use type "multi_select_with_counts" (NOT plain "multi_select"). This lets the instructor specify how many questions of each task type they want — e.g. "3 composition questions, 2 horizontal-shift questions" instead of just picking which task types apply.

   Shape for student_tasks with counts:
   {
     id: "student_tasks",
     label: "What students compute or identify",
     type: "multi_select_with_counts",
     default: [
       { value: "identify_transformation", count: 2 },
       { value: "apply_transformation", count: 1 }
     ],
     minSelections: 1,
     options: [
   ...
   The default array entries are { value, count } objects. count is a number 1-5 indicating the default question count for that task. minSelections still applies (number of entries with count > 0 must meet it).

   All OTHER multi_select fields (representation_forms, function_types) stay as plain "multi_select" — counts only make sense for student_tasks because that's where instructors think in terms of "I want N of each type."

   PER-TASK DIFFICULTY: each default entry includes a "difficulty" field
   (one of "easy", "medium", "hard", "mix_easy", "mix_balanced", "mix_hard").
   Use "medium" for all defaults; the instructor changes per-task difficulty
   in the form UI. The template must also include a DIFFICULTY RUBRIC block
   in conditional_quality_blocks that defines what Easy / Medium / Hard mean
   for each task type in this section. Use this shape:

       {
         condition: { field: "student_tasks", includes: "<any_task_value>" },
         content: \`
   DIFFICULTY RUBRIC for §<section> (<title>):
   <task_id_1>:
   - Easy: <what makes it easy in THIS section>
   - Medium: <middle complexity for THIS section>
   - Hard: <textbook-end-of-section level for THIS section>

   (continue for each task in this section's student_tasks options)
   \`
       }

   The rubric is triggered when any task is selected, so it always fires
   when difficulty matters. Without it, the model has no consistent definition
   of difficulty for this section.
...
**conditional_quality_blocks** — Prompt blocks that fire when specific conditions match. Use this when a particular instructor choice triggers a known quality risk in this section.

CRITICAL: The predicate type depends on whether the field is multi_select or single_select. Look up the field's "type" in the fields array above and use the right shape:

  For multi_select fields (e.g. function_types, representation_forms, student_tasks):
    { condition: { field: "<multi_select_field_id>", includes: "<value>" }, content: "..." }
    → fires when the user has selected that value (the answer is an array).
    Example: \`{ condition: { field: "function_types", includes: "piecewise" }, content: "..." }\`

  For single_select fields (e.g. primary_focus, domain_complexity, graphs_option, question_type):
    { condition: { field: "<single_select_field_id>", equals: "<value>" }, content: "..." }
    → fires when the user has chosen that value (the answer is a string).
    Example: \`{ condition: { field: "primary_focus", equals: "composition" }, content: "..." }\`

DO NOT use \`includes\` on a single_select field. The block will silently fail to fire because single_select values are strings, not arrays. Always check the field's type before writing the condition.

If you can't think of a real quality risk specific to this section, leave the array empty []. Empty is better than firing the wrong block.
...
**reference_examples** — Extract 2 to 4 representative worked examples FROM the uploaded textbook materials. These anchor the style of all future generations. Each entry is an object of shape:

  {
    stem: "the question as it appears in the textbook, rewritten in mini-syntax",
    choices: ["choice 1", "choice 2", "choice 3", "choice 4"],   // omit or use [] if FR
    answer: "the textbook's correct answer (full text, not a letter)",
    explanation: "a one-sentence summary of the textbook's solution style"
  }

Pick examples that span the section's range — one easy/canonical example, one medium-difficulty example, one example showing an edge case or common student error. Pick from the materials provided, not from your training data. If the materials don't contain explicit example problems, leave reference_examples as []. Do not invent examples that aren't grounded in the uploaded materials.

Each example's stem should be rewritten in MINI-SYNTAX (the same rules as everywhere else — no Unicode, no LaTeX backslashes). The downstream prompt will include these examples verbatim in the generation prompt, so anything you put here will be seen by Sonnet during every future question generation for this section. Concise, clean, style-anchoring examples are gold; verbose or sloppy ones poison the well.

═══════════════════════════════════════════════════════════
NOTATION INSIDE STRING VALUES
═══════════════════════════════════════════════════════════

Wherever your output contains math inside string values (placeholder text, UI labels in options arrays, conditional_quality_blocks content, notation_additions, error messages — ANY string in the template), use mini-syntax — NEVER LaTeX backslash commands AND NEVER Unicode math symbols:
...
- Function composition: write "f composed with g" or "(f o g)(x) = f(g(x))" — NEVER "∘" (Unicode composition ring) or "f ∘ g"
- Set operations: write "union" or "U" — never ∪ ∩ ∈ ∉
- Infinity: infinity — never ∞
- Arrows: write "->" — never → ← ↔
- Other Greek/math glyphs (delta, epsilon, sigma, sum, product, etc.) — spell them out

This rule applies to EVERY string in the template, including labels the instructor will see in the UI. The downstream renderer expects mini-syntax everywhere; Unicode symbols leak through to question text and break.
