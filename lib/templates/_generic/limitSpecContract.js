...
- "classifyDiscontinuity" → "continuous" | "removable" | "jump" | "infinite"
                            (the type of behavior at x = at).
...
"continuous_except" | "with_reasons".
...
isContinuous / classifyDiscontinuity): still author 4 plausible choices. The
system formats the DERIVED value and matches it to one of your choices. If the
derived truth is not among your choices, the question is REJECTED — so make sure
the correct value is present verbatim (e.g. include "4", "does not exist (DNE)",
"infinity", "removable" as appropriate).

For Multiple Choice GLOBAL continuity asks (discontinuitySet / removableSet /
limitDNESet): supply DISTRACTORS ONLY — plausible but WRONG statements. The
system composes the correct statement from the derived set and injects it as the
answer key (you cannot set the correct answer). Provide at least 2 distractor
choices.`;
