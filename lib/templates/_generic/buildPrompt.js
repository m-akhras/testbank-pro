  ...
  for (const field of template.fields) {
    ...
    if (field.type === "multi_select_with_counts" && field.minSelections) {
      ...
      if (!Array.isArray(value)) {
        throw new Error(`Field ${field.id} must be an array of {value, count} entries`);
      }
...
export function buildPrompt(template, answers, options = {}) {
  ...
  // student_tasks may be either multi_select (string[]) or multi_select_with_counts ([{value, count}, ...])
  ...
  const studentTasks = studentTasksHasCounts
