...
SCHEMA A2 — Overlay (two or more functions on the same axes):

This is the textbook "before/after" pattern for transformation questions. Show the parent f(x) and the transformed h(x) on one set of axes so the student sees both curves at once. The parent is conventionally drawn DASHED so the student can tell it apart from the transformed curve.

{
  "type": "overlay",
  "fns": [
    { "fn": "sqrt(x)",                "label": "f(x)", "color": "blue", "dashed": true  },
    { "fn": "-2*sqrt(x - 1) + 3",     "label": "h(x)", "color": "red",  "dashed": false }
  ],
  "xDomain": [-1, 8],
  "yDomain": [-3, 5],
  "showAxisNumbers": true,
...
Required: type ("overlay"), fns (array of at least one { fn } entry)
Per-entry optional: label (text drawn near the curve), color ("blue" | "red" | "green" | "orange"), dashed (boolean)
Global optional: xDomain, yDomain, holes, points, showAxisNumbers, showGrid

Convention: when overlaying parent + transformed, list the parent FIRST with dashed: true and label "f(x)" (color blue), then the transformed SECOND with dashed: false and label "h(x)" (color red). Choose xDomain and yDomain wide enough to show BOTH curves' interesting features.

USE OVERLAY for:
- "The graph of f is shown (dashed). Which equation describes the transformed graph h(x) shown solid?" — verbal stem, overlay graph, text choices
- "Compare the graph of y = x^2 (dashed) and y = -(x-1)^2 + 2 (solid). Which transformations produce h from f?" — same idea
- Any question where the student needs to SEE the original to understand what happened

DO NOT use overlay for:
- Pure verbal-to-algebraic questions (write the equation for "shift 3 right" — no graph needed)
- Composition / decomposition / arithmetic-combination questions (algebraic, not visual)
- Questions where the parent function is unfamiliar enough to be confusing on its own
...
FORBIDDEN CHARACTERS in "fn" strings (these cause the renderer to fail silently — blank graph, no error message):

NEVER put any of these inside a fn string:
- Unicode math glyphs: π ∞ ≤ ≥ ≠ ∘ ∪ → ↔ • × ÷ ± √
- LaTeX delimiters or commands: \\( \\) \\frac \\sqrt \\sin \\pi \\infty
- Any backslash anywhere: \\, \\\\, \\anything

CORRECT vs WRONG fn strings:
- "sin(x - pi)"          NOT  "sin(x - π)"
- "2*pi"                 NOT  "2π" or "2*π"
- "sqrt(x + 3)"          NOT  "√(x + 3)" or "\\sqrt{x+3}"
- "1/(x - 2)"            NOT  "1÷(x-2)"
- "-2*(x + 1)^3 + 4"     NOT  "−2·(x + 1)³ + 4"
- "exp(-x^2)"            NOT  "e^(-x²)"

These rules apply specifically to the "fn" field (and "fn" fields inside "pieces" for piecewise). The renderer parses fn as a math expression and will produce a blank graph if it encounters any character it doesn't recognize. Unicode glyphs and LaTeX backslashes are the most common cause of blank graphs in this codebase.
...
TRANSFORMATION QUESTIONS — three valid patterns (use the right one for the task):

PATTERN A — Stem graph, text choices.
The stem shows ONE graph (usually the parent function f). The student reads the graph and the text choices describe transformations or new equations.
Use when: the question is "what transformation was applied?" or "which equation describes this graph?". The student's job is to NAME or WRITE the transformation, not visually identify it.

Example:
{
  "question": "The graph above shows h(x). Relative to f(x) = x^2, which transformation produces h(x)?",
  "hasGraph": true,
  "graphConfig": { "type": "single", "fn": "(x - 3)^2", "xDomain": [-1, 7], "yDomain": [-1, 9], "points": [[3, 0]] },
  "choices": ["Shift right 3 units", "Shift left 3 units", "Reflect about the x-axis", "Vertical stretch by 3"],
  "answer": "Shift right 3 units"
}

PATTERN B — Graph choices, no stem graph.
No graph in the stem. The 4 choices are graph objects. The student picks the graph that matches the equation or description.
Use when: the question is "which of these graphs is y = ...?" or "which graph represents the result of these transformations?". The student's job is to visually identify the correct graph from candidates.

Example:
{
  "question": "Which graph represents y = (x - 2)^2 - 3?",
  "choices": [
    { "graphConfig": { "graphType": "single", "fn": "(x - 2)^2 - 3", "xDomain": [-1, 5], "yDomain": [-4, 4] } },
    { "graphConfig": { "graphType": "single", "fn": "(x + 2)^2 - 3", "xDomain": [-5, 1], "yDomain": [-4, 4] } },
    { "graphConfig": { "graphType": "single", "fn": "(x - 2)^2 + 3", "xDomain": [-1, 5], "yDomain": [0, 8] } },
    { "graphConfig": { "graphType": "single", "fn": "-(x - 2)^2 - 3", "xDomain": [-1, 5], "yDomain": [-8, 0] } }
  ],
  "answer": "A"
}

PATTERN C — Stem graph AND graph choices (before/after — the most powerful pattern).
The stem shows the parent f(x). The 4 choices show candidate transformed graphs h(x). The student sees the original and picks the right transformation visually.
Use when: the transformation is complex enough that visual comparison helps, OR when the parent function is unfamiliar enough that students need to see it. This is the textbook pattern for transformation questions and the most pedagogically valuable for §1.3.

Example:
{
  "question": "The graph of f(x) is shown above. Which of the following is the graph of h(x) = -f(x - 2) + 1?",
  "hasGraph": true,
  "graphConfig": { "type": "single", "fn": "sqrt(x)", "xDomain": [-1, 6], "yDomain": [-1, 3], "fnLabel": "f(x)", "points": [[0, 0]] },
  "choices": [
    { "graphConfig": { "graphType": "single", "fn": "-sqrt(x - 2) + 1", "xDomain": [-1, 8], "yDomain": [-3, 2], "fnLabel": "h(x)", "points": [[2, 1]] } },
    { "graphConfig": { "graphType": "single", "fn": "sqrt(x - 2) + 1",  "xDomain": [-1, 8], "yDomain": [-1, 5], "fnLabel": "h(x)", "points": [[2, 1]] } },
    { "graphConfig": { "graphType": "single", "fn": "-sqrt(x + 2) + 1", "xDomain": [-3, 6], "yDomain": [-3, 2], "fnLabel": "h(x)", "points": [[-2, 1]] } },
    { "graphConfig": { "graphType": "single", "fn": "-sqrt(x - 2) - 1", "xDomain": [-1, 8], "yDomain": [-4, 0], "fnLabel": "h(x)", "points": [[2, -1]] } }
  ],
  "answer": "A"
}

GUIDANCE for §1.3 (transformations, composition, combining):
- For transformation questions with FAMILIAR parents (x^2, sqrt(x), |x|, sin(x), x^3): prefer Pattern B (text or graph choices). Students recognize the parent without needing to see it.
- For transformation questions with PIECEWISE parents or unusual graphs: use Pattern C (stem + graph choices). Students need to see the specific f(x) before they can identify the transformation.
- For "identify the transformation" questions where the answer is a name or description: use Pattern A (stem only, text choices).
- For "compute (f o g)(x)" or "evaluate (f o g)(3)": no graph needed. Pure text question.

Mix patterns across a question set. A typical §1.3 batch of 10 questions might use Pattern A for 3, Pattern B for 4, Pattern C for 2, and no-graph for 1.
