...
NOTATION in stems: write piecewise functions as prose with explicit conditions, e.g. "f(x) = x^2 + 2 if x < 0, and f(x) = x if x >= 0". Stems may say "the piecewise-defined function" followed by the prose form. (Brace mini-syntax renders nicely in Canvas/web but currently breaks Word and Canvas answer-key import; revisit when those export paths handle it.)
...
`
    ...
    notation_additions: [
      ...
      `PIECEWISE FUNCTIONS in stems: write as prose with explicit conditions, e.g. "f(x) = x^2 + 2 if x < 0, and f(x) = x if x >= 0". (Brace mini-syntax { ... ; ... } renders as stacked cases in Canvas student-view and web preview, but currently breaks Word export and Canvas answer-key import, so use prose until those paths are fixed.)`,
