...
NOTATION in stems: write piecewise functions as prose with explicit conditions, e.g.:
"f(x) = x^2 + 2 if x < 0, and f(x) = x if x >= 0"
OR using mini-syntax that renders correctly:
"f(x) = { x^2 + 2 if x < 0; x if x >= 0 }"

The renderer's piecewise display is a work in progress; prefer prose form for stems unless the template handles curly-brace mini-syntax cleanly. Stems may say "the piecewise-defined function" followed by the prose form.
...
`
    ...
    notation_additions: [
      ...
      `PIECEWISE FUNCTIONS in stems: write either as prose ("f(x) = x^2 + 2 if x < 0, and f(x) = x if x >= 0") or with mini-syntax curly braces. The renderer's stacked-cases display is a work-in-progress; prose form is currently more reliable.`,
