...
// with the 3 required *_values enums). The `question_style` router selects one
// of four flavors. ONE style — "graph_continuity" — emits a declared limitSpec
// plus CONTINUITY asks (isContinuous / classifyDiscontinuity point asks, and the
// discontinuitySet / removableSet / limitDNESet global asks), routed through
// applyLimitSpec → the SAME §2.2 engine + guards (phantom-VA, interior-pole) and
// the system-derived answer key. The other three styles are symbolic (no
// limitSpec → applyLimitSpec no-ops; the model computes and states the answer).
...
export const calc1_2_5_template = {
  ...
  fields: [
    ...
    },
    {
      ...
      type: "single_select",
      default: "graph_continuity",
      options: [
        { value: "graph_continuity", label: "From a given graph: find / classify discontinuities, where the limit DNE" },
        { value: "find_constant", label: "Find the constant(s) that make a piecewise function continuous" },
        { value: "classify_explain", label: "Explain why f is discontinuous at a given number a" },
    ...
    },
    {
      ...
      type: "multi_select",
      default: ["identify_discontinuities_from_graph", "classify_discontinuity"],
      minSelections: 1,
      options: [
        { value: "identify_discontinuities_from_graph", label: "Identify discontinuities from a graph" },
        { value: "classify_discontinuity", label: "Classify a discontinuity (removable / jump / infinite)" },
...
- POINT asks: "isContinuous" (at a) → continuous/discontinuous; "classifyDiscontinuity" (at a) → continuous/removable/jump/infinite. For Multiple Choice POINT asks, author normal choices that INCLUDE the derived value (e.g. include "removable", "jump", "infinite", "continuous").
...
`
      ...
      {
        // ── ROUTING: classify_explain → symbolic, model computes the answer ──
        condition: { field: "question_style", equals: "classify_explain" },
        content: `
§2.5 CLASSIFY-AND-EXPLAIN STYLE — SYMBOLIC, COMPUTE AND STATE THE ANSWER:
Do NOT emit a limitSpec for this style. Ask "Explain why the function is discontinuous at the given number a" for a concrete function — a rational with a pole or hole (e.g. 1/(x+2) at a=-2; (x^2-1)/(x-1) with f(1) redefined), or a small piecewise. Compute the answer yourself.
Method to show: check the THREE continuity conditions at a — (1) f(a) is defined, (2) lim x→a f(x) exists, (3) the two are equal. Name which condition fails and therefore the KIND: removable (limit exists but f(a) undefined or different), jump (one-sided limits differ), infinite (function unbounded near a). State the kind explicitly.
DISTRACTORS (for Multiple Choice): name the wrong kind (call a removable hole "infinite", or a jump "removable"); claim continuity because f(a) is defined while ignoring that the limit differs.
`
      },
      {
        // ── ROUTING: analyze_piecewise → symbolic, model computes the answer ──
        condition: { field: "question_style", equals: "analyze_piecewise" },
        content: `
§2.5 ANALYZE-PIECEWISE STYLE — SYMBOLIC, COMPUTE AND STATE THE ANSWER:
Do NOT emit a limitSpec for this style. Either ask "Show that f is continuous on (-infinity, infinity)" for a piecewise that IS continuous (e.g. 1 - x^2 for x <= 1, ln(x) for x > 1), OR ask "Find the numbers at which f is discontinuous; at which of these is f continuous from the right, from the left, or neither?" for a 3-branch piecewise with breaks. Compute the answer yourself.
Method to show: each piece is continuous on its interval; check each breakpoint by comparing lim x→a^- f(x), lim x→a^+ f(x), and f(a). If all three agree → continuous there. Otherwise it is discontinuous; report ONE-SIDED continuity: continuous from the LEFT if f(a) = lim x→a^- f(x); from the RIGHT if f(a) = lim x→a^+ f(x); NEITHER if it matches no side.
DISTRACTORS (for Multiple Choice): swap left/right one-sided continuity; claim "neither" where one side actually matches; miss a breakpoint that is actually discontinuous.
`
      },
      {
...
`
    ...
    primary_task_values: [
      "identify_discontinuities_from_graph", "classify_discontinuity",
      "find_limit_dne_set", "find_constant_for_continuity",
      "explain_discontinuity", "analyze_piecewise_continuity",
