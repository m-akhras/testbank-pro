...
`
    ...
    notation_additions: [
      ...
      `LOG GRAPH RENDERING: when showing y = log_b(x) on a graph, write fn: "log(x)/log(b)" in graphConfig (change-of-base form, since the renderer's log is natural log). Examples: y = log_3(x) -> fn: "log(x)/log(3)"; y = ln(x) -> fn: "log(x)"; y = log_10(x) -> fn: "log(x)/log(10)". NEVER render y = log_b(x) as isolated points or a discrete sequence — it is always a continuous curve. Domain x > 0 is handled by the xDomain rule.`
    ],
