export const calc1_1_4_template = {
  ...
  fields: [
    ...
    },
    {
      ...
      default: [
        { value: "simplify_exponent_expression", count: 1, difficulty: "medium" },
        { value: "evaluate_exponential", count: 1, difficulty: "medium" },
        { value: "identify_growth_decay", count: 1, difficulty: "medium" },
        { value: "match_equation_to_graph", count: 1, difficulty: "medium" },
        { value: "transformation_of_exponential", count: 1, difficulty: "medium" }
      ],
      minSelections: 1,
      options: [
  ...
  section_specific_rules: {
    ...
    graph_eligible_tasks: [
      ...
      "transformation_of_exponential",
      "write_transformation_equation",
    ...
    conditional_quality_blocks: [
      {
        condition: { field: "student_tasks", includes: "simplify_exponent_expression" },
        content: `
DIFFICULTY RUBRIC for §1.4 (Exponential Functions):
The STUDENT TASKS block above tells you, per-task, how many questions to generate at Easy / Medium / Hard. This block defines what those labels MEAN for each §1.4 task. Pick the appropriate complexity per question to honor the requested distribution. Set the per-question "difficulty" field in your output to "Easy", "Medium", or "Hard" matching the level you actually wrote at.

simplify_exponent_expression (Stewart Ex 1-2):
- Easy: one law, integer base, integer exponents (e.g. 2^4 * 2^(-1) / 2^2)
- Medium: two laws combined, includes negative or fractional exponents (e.g. (2*x^2 * y) / (3*x^(-2) * y)^2)
- Hard: nested negative exponents, mixed roots, symbolic variables (e.g. b^3 * (3*b^(-1))^(-2); sqrt(a*sqrt(b)) / cbrt(a*b))

evaluate_exponential:
- Easy: b^x for small integer b and small integer x (e.g. 5^(-2), 3^4)
- Medium: fractional base or rational exponent (e.g. (1/2)^(-3), 16^(3/4))
- Hard: composite or chained (e.g. ((1/4)^(-1))^(1/2), 8^(2/3) - 27^(-1/3))

identify_growth_decay:
- Easy: integer or simple decimal base, identify increasing/decreasing
- Medium: compare two functions and identify which grows faster
- Hard: identify growth/decay AND the horizontal asymptote and y-intercept of a transformed exponential

match_equation_to_graph:
- Easy: y = b^x for b in {2, 3, 5, e, 10} or their reciprocals (no shifts, no reflections)
- Medium: one transformation applied (single shift OR single reflection)
- Hard: two or more transformations (shift + reflection, or shift + stretch)

transformation_of_exponential (Stewart Ex 9-14):
- Easy: single transformation of y = e^x or y = b^x (one of shift, reflect, stretch)
- Medium: two transformations (Stewart Ex 9 g(x) = 3^x + 1 style)
- Hard: three or more transformations or non-canonical bases (Stewart Ex 13 y = 1 - (1/2)*e^(-x) style)

write_transformation_equation (Stewart Ex 15):
- Easy: single transformation described verbally (shift down, reflect about x-axis)
- Medium: two transformations described in sequence
- Hard: three transformations OR a non-axis reference line

reflection_about_line (Stewart Ex 16):
- Easy: reflection about a coordinate axis (y-axis or x-axis)
- Medium: reflection about y = c or x = c for small integer c
- Hard: reflection about a non-integer line OR a sequence of reflections about different lines

find_domain (Stewart Ex 17-18):
- Easy: domain is all reals (because e^anything is always defined and positive)
- Medium: domain has one restriction (denominator zero OR sqrt argument)
- Hard: nested exponential restrictions requiring log analysis or compound conditions

compound_interest:
- Easy: annual compounding, find balance after t years given P and r
- Medium: compounded n times per year, OR continuous compounding (one of these)
- Hard: compare annual vs continuous over a given time, OR solve for the time to reach a target balance

population_growth_decay:
- Easy: direct plug-in (given the formula, find P(t) for a specific t)
- Medium: identify the growth rate from two data points, OR find half-life given decay
- Hard: solve for t given a target value (requires basic exponential equation solving)
...
`
    ...
    notation_additions: [
      ...
      `LABEL CONVENTION FOR GRAPHS: when including a graph (single, overlay, or piecewise), the fnLabel (or per-entry label in overlay) MUST be a short identifier — "y", "f(x)", "g(x)", "h(x)" — never the full equation. The renderer draws labels as plain SVG text with no math formatting, so a label like "y = ((1)/(2))^x" displays as literal ASCII text on the graph, not as rendered math. Keep labels short. The equation belongs in the question stem, not on the curve.`,
