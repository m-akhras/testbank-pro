...
const GRAPH_READING_BLOCK = `
§2.2 GRAPH-READING STYLE — EMIT A LIMIT SPEC, DO NOT STATE THE ANSWER:
FUNCTION LETTER (MANDATORY): the function MUST be called f. Write "f" and "f(x)" everywhere in the question text — NEVER h, g, y, or any other letter. The graph is labeled f and the derived answer key says "f", so any other letter (e.g. "the function h", "h(x)") contradicts the graph and is REJECTED.
...
- Declare a vertical asymptote at x=a ONLY where a segment expression actually diverges to +infinity or -infinity — i.e. a RATIONAL whose denominator is zero at a while the numerator is nonzero there. Polynomials, roots (sqrt/cube_root), exponentials, and logarithms (inside their domain) do NOT have vertical asymptotes — declare NONE for them.
- A declared VA at x=a MUST be consistent with the segments: the segment covering or bordering a must genuinely blow up as x approaches a. NEVER declare a VA at an x where the segment value is finite (e.g. sqrt(x+5) at x=2 ≈ 2.6, or x^2+2 anywhere) — the spec validator REJECTS such questions.
- The NUMBER of vertical asymptotes allowed is governed by the "VERTICAL ASYMPTOTE COUNT" rule below (set by the instructor's VA mode). The divergence-consistency rule above is about VALIDITY and always applies regardless of mode.

${LIMIT_SPEC_CONTRACT}
`;

const PIECEWISE_EXISTENCE_BLOCK = `
...
const MIXED_BLOCK = `
§2.2 MIXED STYLE — YOU CHOOSE EACH QUESTION'S STYLE PER QUESTION:
Generate a VARIED batch, choosing for EACH question one of the four §2.2 styles below (graph_reading, piecewise_existence, infinite_limit, find_va), guided by the instructor's selections:
- Representation forms: if "Graphical" is selected, graph_reading questions are allowed (and encouraged). If "Graphical" is NOT selected, do NOT produce any graph question — use only the symbolic styles (piecewise_existence / infinite_limit / find_va).
- Function types & difficulty steer which symbolic style fits (rational → infinite_limit / find_va; piecewise → piecewise_existence).
- Suggested mix when graphical is available: roughly half graph_reading and half symbolic, varied across the batch (this is guidance, not a rigid count). When graphical is not available: all symbolic.
...
${GRAPH_READING_BLOCK}
${PIECEWISE_EXISTENCE_BLOCK}
${INFINITE_LIMIT_BLOCK}
${FIND_VA_BLOCK}
`;
...
This question must have NO vertical asymptotes. Every segment must be finite across its interval. Do NOT declare any verticalAsymptotes (for graph_reading, omit the verticalAsymptotes array or leave it empty); for symbolic styles, the function must not have any pole. (The divergence-consistency rule still applies: you could not declare one here anyway, because there is nothing that diverges.)
`
      },
      {
...
Use AT MOST ONE vertical asymptote, and only where a segment genuinely diverges (a rational whose denominator is zero, numerator nonzero, at that x). Many §2.2 graphs have NONE — do not add one just to have one. If you do include the single asymptote, declare it at a segment boundary (split the function so the pole sits at a from/to, never strictly inside a segment's interior).
`
      },
      {
...
You MAY include several vertical asymptotes — but EACH must be real: a segment must genuinely diverge at that boundary (denominator zero, numerator nonzero). Declare every VA at a SEGMENT BOUNDARY: split the function into segments around each asymptote so each pole sits at a from/to, never inside a segment's interior (the validator REJECTS a pole strictly inside a segment). Build genuinely multi-asymptote functions — e.g. a piecewise with rational branches that each have their own boundary pole. Do not pad with fake asymptotes where the function is finite.
`
      }
    ],
