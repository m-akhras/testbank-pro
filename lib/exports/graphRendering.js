...
export function renderGraphToSVG(graphConfig, width = 480, height = 300) {
  ...
  if (cfg.type === "mapping") {
    ...
    const defs = mk("defs", {});
    const marker = mk("marker", { id:"arrowhead", markerWidth:"8", markerHeight:"6",
      refX:"8", refY:"3", orient:"auto" });
    const poly = mk("polygon", { points:"0 0, 8 3, 0 6", fill:"#334155" });
    ...
    arrows.forEach(([from, to]) => {
      ...
      svg.appendChild(mk("line", { x1:String(x1), y1:String(y1), x2:String(x2), y2:String(y2),
        stroke:"#334155", "stroke-width":"1.5", "marker-end":"url(#arrowhead)" }));
    });
