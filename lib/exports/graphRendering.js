...
export function renderGraphToSVG(graphConfig, width = 480, height = 300) {
  ...
  if (cfg.type === "venn") {
    ...
    if (printMode) {
      // Hatching patterns for B&W print — natural cross-hatch where circles overlap
      const pA = mk("pattern", { id:"hatch-A", patternUnits:"userSpaceOnUse",
        width:"6", height:"6", patternTransform:"rotate(45)" });
      pA.appendChild(mk("line", { x1:"0", y1:"0", x2:"0", y2:"6",
        stroke:"#000", "stroke-width":"0.8" }));
      defs.appendChild(pA);

      const pB = mk("pattern", { id:"hatch-B", patternUnits:"userSpaceOnUse",
        width:"6", height:"6", patternTransform:"rotate(-45)" });
      pB.appendChild(mk("line", { x1:"0", y1:"0", x2:"0", y2:"6",
        stroke:"#000", "stroke-width":"0.8" }));
      defs.appendChild(pB);

      const pC = mk("pattern", { id:"hatch-C", patternUnits:"userSpaceOnUse",
        width:"6", height:"6" });
      pC.appendChild(mk("line", { x1:"0", y1:"3", x2:"6", y2:"3",
        stroke:"#000", "stroke-width":"0.8" }));
      defs.appendChild(pC);

      // Cross-hatch for "AandB"-only shading where A and B are not individually shaded
      const pAB = mk("pattern", { id:"hatch-AB", patternUnits:"userSpaceOnUse",
        width:"6", height:"6" });
      pAB.appendChild(mk("line", { x1:"0", y1:"0", x2:"6", y2:"6",
        stroke:"#000", "stroke-width":"0.8" }));
      pAB.appendChild(mk("line", { x1:"0", y1:"6", x2:"6", y2:"0",
        stroke:"#000", "stroke-width":"0.8" }));
      defs.appendChild(pAB);
    }
    ...
    if (shaded.includes("AorB") || shaded.includes("A") || shaded.includes("AnotB")) {
      shade(printMode ? "url(#hatch-A)" : colorA, "clipA", printMode ? 1 : 0.35);
    }
    if (shaded.includes("AorB") || shaded.includes("B") || shaded.includes("BnotA")) {
      shade(printMode ? "url(#hatch-B)" : colorB, "clipB", printMode ? 1 : 0.35);
    }
    ...
    if (shaded.includes("AandB")) {
      ...
      g.appendChild(mk("circle", {
        ...
        fill: printMode ? "url(#hatch-AB)" : "#8b5cf6",
        "fill-opacity": printMode ? "1" : "0.45"
      }));
      svg.appendChild(g);
    }
    ...
    // Element labels inside each region. In print mode, every label gets a
    // thick white stroke painted FIRST (paint-order:stroke), then the black
    // fill on top — creating a clean halo so the digits read cleanly over
    // crosshatch fills in the intersection. The same attributes are harmless
    // for labels outside the hatched region.
