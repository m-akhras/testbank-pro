...
export function renderStatChartToSVG(chartConfig, width=480, height=300) {
  ...
  // ── Scatter Plot ──────────────────────────────────────────────────────────
    ...
    // Y-intercept marker at (0, b₀): dashed guide + red dot + value label.
    // Drawn last so it sits on top of the regression line and gridlines.
    if (hasRegression) {
      const ix = xScale(0);
      const iy = yScale(intercept);
      // Only render if (0, intercept) is actually in view (it should be, since
      // we forced the domain to include it, but guard anyway).
      if (ix >= 0 && ix <= iW && iy >= 0 && iy <= iH) {
        g.append("line")
          .attr("x1", 0).attr("y1", iy).attr("x2", ix).attr("y2", iy)
          .attr("stroke", COL.muted).attr("stroke-width", 1.2).attr("stroke-dasharray", "4,3");
        g.append("circle")
          .attr("cx", ix).attr("cy", iy).attr("r", 4)
          .attr("fill", "#ef4444").attr("stroke", "#fff").attr("stroke-width", 1.5);
        const bRounded = Math.round(intercept * 100) / 100;
        // Place the label to the right of the dot and slightly above; clamp so
        // it doesn't run off the right edge of the plot area.
        const labelX = Math.min(ix + 8, iW - 60);
        const labelY = Math.max(iy - 6, 12);
        g.append("text")
          .attr("x", labelX).attr("y", labelY)
          .attr("font-size", 10).attr("font-weight", "600").attr("fill", COL.text)
          .text(`y-int: ${bRounded}`);
      }
    }
