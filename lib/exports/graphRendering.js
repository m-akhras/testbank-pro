...
export function renderGraphToSVG(graphConfig, width = 480, height = 300) {
  ...
  if (cfg.type === "venn") {
    ...
    // In print mode each shaded circle gets a translucent light-gray fill —
    // when both A and B are shaded the overlap stacks to a darker gray, so the
    // intersection is naturally distinguished without any pattern fill. This
    // matches the visual style of the mapping diagram's ovals (light gray on
    // black outline) and keeps element labels readable as plain black text.
    if (shaded.includes("AorB") || shaded.includes("A") || shaded.includes("AnotB")) {
      shade(printMode ? "#e8e8e8" : colorA, "clipA", printMode ? 0.5 : 0.35);
    }
    if (shaded.includes("AorB") || shaded.includes("B") || shaded.includes("BnotA")) {
      shade(printMode ? "#e8e8e8" : colorB, "clipB", printMode ? 0.5 : 0.35);
    }
    ...
    if (shaded.includes("AandB")) {
      ...
      g.appendChild(mk("circle", {
        ...
        fill: printMode ? "#e8e8e8" : "#8b5cf6",
        "fill-opacity": printMode ? "0.5" : "0.45"
      }));
      svg.appendChild(g);
    }
    ...
    // Element labels — plain text in both modes. The print-mode regions are
    // light-gray translucent fills, so plain black text reads cleanly without
    // any stroke halo.
