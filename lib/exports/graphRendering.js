...
export function evalFn(exprRaw, xVal) {
  try {
    let expr = String(exprRaw ?? "")
      // operators
      .replace(/\^/g, "**")
      // FIX: JavaScript strict mode forbids a unary operator (-/+) immediately
      // before **. e.g. "-(x-1)**2" and "-x**2" are SyntaxErrors. The math we
      // want is "-((x-1)**2)" and "-(x**2)" — exponent binds tighter than unary
      // minus. We wrap (group)**n and bare_ident**n in parens so any preceding
      // sign is unambiguous to the JS parser.
      .replace(/(\([^()]*\))\s*\*\*\s*(-?\d+(?:\.\d+)?|\([^()]+\))/g, "($1**$2)")
      .replace(/(?<![.a-zA-Z0-9_])([a-zA-Z][a-zA-Z0-9]*)\s*\*\*\s*(-?\d+(?:\.\d+)?|\([^()]+\))/g, "($1**$2)");
    ...
    expr = expr
      ...
      .replace(/([)\d])\s*\(/g, "$1*(")
      // implicit multiplication: )letter — e.g. (2)sqrt → (2)*sqrt, (x+1)cos(x) → (x+1)*cos(x).
      // The two passes above cover digit→letter/paren and )→(, but miss )→letter, which the
      // LLM hits whenever it writes a fraction coefficient like (1)/(2)sqrt(...).
    ...
    if (typeof window !== "undefined" && window.console) {
      console.warn("[graphRendering.evalFn] Expression failed to evaluate:", exprRaw, "—", e.message);
    }
    return NaN;
  }
}
