...
export function renderGraphToSVG(graphConfig, width = 480, height = 300) {
  ...
  if (cfg.type === "mapping") {
    ...
    // a single black arrowhead and disambiguates crossings via dasharray.
    const palette = ["#3b82f6","#ef4444","#8b5cf6","#f59e0b","#10b981","#ec4899"];
    const printPatterns = ["", "6,3", "2,3", "8,3,2,3", "1,2", "10,2,1,2"];
    ...
    function renderOneMapping(mc, x0, x1) {
      ...
      // arrows — color per domain index, or B&W with dasharray in print mode
      arrows.forEach(([from, to]) => {
        ...
        const lineAttrs = {
          x1:String(x1c), y1:String(y1c), x2:String(x2c), y2:String(y2c),
          "stroke-width":"2",
        };
        if (printMode) {
          lineAttrs.stroke = "#000000";
          const dashArray = printPatterns[from % printPatterns.length];
          if (dashArray) lineAttrs["stroke-dasharray"] = dashArray;
          lineAttrs["marker-end"] = "url(#arrowhead-bw)";
        } else {
          lineAttrs.stroke = palette[colorIdx];
          lineAttrs["marker-end"] = `url(#arrowhead-${colorIdx})`;
        }
        svg.appendChild(mk("line", lineAttrs));
      });
    }
  ...
  if (cfg.type === "venn") {
    ...
    (cfg.elementsA || []).forEach((el, i) => {
      svg.appendChild(mk("text", {
        x: String(cA.x - r * 0.55), y: String(cy - 10 + i * 20),
        "text-anchor": "middle", "font-size": "16", "font-family": "sans-serif", fill: "#1e293b"
      }, el));
    });
    (cfg.elementsB || []).forEach((el, i) => {
      svg.appendChild(mk("text", {
        x: String(cB.x + r * 0.55), y: String(cy - 10 + i * 20),
        "text-anchor": "middle", "font-size": "16", "font-family": "sans-serif", fill: "#1e293b"
      }, el));
    });
    (cfg.elementsAB || []).forEach((el, i) => {
      svg.appendChild(mk("text", {
        x: String(cx), y: String(cy - 10 + i * 20),
        "text-anchor": "middle", "font-size": "16", "font-family": "sans-serif", fill: "#1e293b"
      }, el));
    });
