...
import { wrapExponentBases } from "../utils/exprCompile.js";

// Single source of fn-name → JS mapping for evalFn. log = base-10 (this renderer's
// long-standing convention — UNCHANGED), ln = natural log, log10/log2 explicit.
// `sec/csc/cot` expand to reciprocals. Applied in ONE longest-first pass so a
// substituted "Math.log" (from ln) is never re-matched by the shorter "log".
const FN_MAP = {
...
// (?<!Math\.) so a name already substituted by the bars pass ("|x|" → "Math.abs")
// is not re-wrapped into "Math.Math.abs".
const FN_NAME_RE = new RegExp(
  "(?<!Math\\.)\\b(" + Object.keys(FN_MAP).sort((a, b) => b.length - a.length).join("|") + ")\\b",
  "g"
);
// Dev-only throttle so a broken fn is reported once by name, not once per sample.
const _loggedEvalFailures = new Set();

export function evalFn(exprRaw, xVal) {
  try {
    // operators: ^→** then wrap power bases so a preceding unary minus stays
    // outside (e.g. -2^x = -(2^x)). SHARED with compileExpression so the rendered
    // curve and the derived answer key never diverge.
    let expr = wrapExponentBases(String(exprRaw ?? "").replace(/\^/g, "**"));
    // Convert mini-syntax absolute-value bars |...| to Math.abs(...).
    // Must run BEFORE the implicit-mult passes below, which can't safely handle |
    // (it's ambiguous: the same character opens and closes). By converting bars to
    // Math.abs(...) first, "2|x|" becomes "2Math.abs(x)" and the digit->letter pass
    // can then insert the *. Loop handles concatenated patterns like |x+1| + |x-1|.
    let prevBars;
    let absLoopCount = 0;
    do {
      prevBars = expr;
      ...
      absLoopCount++;
      if (absLoopCount > 5) break;
    } while (expr !== prevBars);
    expr = expr
      // implicit multiplication: 2x → 2*x, 3sin → 3*Math.sin, (x+1)(x-1) → (x+1)*(x-1)
      .replace(/(\d)\s*([a-zA-Z])/g, "$1*$2")
      // standalone NUMBER → "(" :  2(x+1) → 2*(x+1). The lookbehind requires the
      // number to be a real literal (not the trailing digits of a function name),
      // so "log10(", "log2(" are NEVER split into "log10*(" / "log2*(".
      .replace(/(?<![A-Za-z_.\d])(\d+(?:\.\d+)?)\s*\(/g, "$1*(")
      // closing ")" → "(" or letter:  (x+1)(x-1) → (x+1)*(x-1), (x+1)cos → (x+1)*cos
      .replace(/\)\s*([a-zA-Z(])/g, ")*$1")
      // implicit multiplication: lowercase ident -> uppercase ident boundary. After the
      // bars conversion above, "x|x|" has become "xMath.abs(x)", which must become
      // "x*Math.abs(x)". The only uppercase letter present at this stage is the "M" of
      // Math.abs (from bars); the Math.* function-name replacements below run afterward.
      .replace(/([a-z])\s*([A-Z])/g, "$1*$2");
    // Function-name normalization — ONE pass, longest-name-first, so a substituted
    // name (e.g. "ln" → "Math.log") is never re-scanned by a later, shorter pattern.
    // The previous sequential chain corrupted ln: "ln"→"Math.log", then "\blog\b"
    // matched the "log" INSIDE "Math.log" → "Math.Math.log10" → threw → blank curve.
    // Semantics (unchanged): ln = natural log; log = log base 10; log10/log2 explicit.
    expr = expr.replace(FN_NAME_RE, (m) => FN_MAP[m])
    ...
    // LOUD in dev (once per unique fn string): a fn that fails to evaluate draws
    // NOTHING — a silently curve-less graph is exactly the ln bug we just fixed, so
    // surface the offending string by name instead of swallowing it.
    if (!_loggedEvalFailures.has(exprRaw)) {
      _loggedEvalFailures.add(exprRaw);
      const isProd = typeof process !== "undefined" && process.env && process.env.NODE_ENV === "production";
      if (!isProd && typeof console !== "undefined" && console.error) {
        console.error("[graphRendering.evalFn] fn failed to evaluate — curve will be BLANK:", JSON.stringify(exprRaw), "—", e.message);
      }
    }
    return NaN;
  }
...
export function renderGraphToSVG(graphConfig, width = 480, height = 300) {
  ...
  const d3  = window.d3;
  // Shallow-copy so the normalization below doesn't mutate the caller's object.
  const cfg = { ...(graphConfig || {}) };
  // Normalize new-convention `graphType` to the legacy `type` field the dispatch
  // reads. The §1.3/§1.4 templates instruct the model to put "single"/"piecewise"
  // under graphType (the new-system field) for choices, while stems use type
  // (the legacy field). The web path bridges this via SinglePiecewiseChoice;
  // the export path passes raw configs straight here, so without this line
  // choice graphs in QTI exports render as axes-only (no curve).
  if (!cfg.type && cfg.graphType) cfg.type = cfg.graphType;

  // §2.3 functionPair → ONE shared graph. Normalize into a piecewise config so it
  // flows the full-fidelity main path (axes, ticks, asymptote breaks, holes,
  // dots, VAs). f's family lives in the standard pieces/holes/points fields drawn
  // blue; g's family rides additive gPieces/gHoles/gPoints fields drawn red by an
  // additive block in the piecewise branch (no-op for every existing config,
  // which has no gPieces). xDomain = union of f/g; VAs merged; we draw our own
  // "f"/"g" curve labels so the per-curve default label is suppressed.
  if (cfg.type === "functionPair") {
    const f = cfg.f || {};
    const gSub = cfg.g || {};
    const fx = Array.isArray(f.xDomain) ? f.xDomain : [-5, 5];
    const gx = Array.isArray(gSub.xDomain) ? gSub.xDomain : [-5, 5];
    const lo = Math.min(fx[0], gx[0]);
    const hi = Math.max(fx[1], gx[1]);
    const span = hi - lo;
    cfg.xDomain = [lo, hi];
    cfg.xTickStep = span <= 12 ? 1 : Math.ceil(span / 12);
    cfg.pieces = f.pieces || [];
    cfg.holes = f.holes || [];
    cfg.points = f.points || [];
    cfg.gPieces = gSub.pieces || [];
    cfg.gHoles = gSub.holes || [];
    cfg.gPoints = gSub.points || [];
    cfg.verticalAsymptotes = [
      ...(f.verticalAsymptotes || []),
      ...(gSub.verticalAsymptotes || []),
    ];
    cfg.fLabel = cfg.fLabel || "f";
    cfg.gLabel = cfg.gLabel || "g";
    cfg.showFnLabel = false; // we draw our own f/g labels below
    cfg.type = "piecewise";
  }

  // ── mapping diagram (Discrete Math Ch.7) ─────────────────────────────────
  // When cfg.second is present, render TWO mappings side-by-side (used for
  // §7.3 composition: f: A→B drawn in the left half, g: B→C in the right).
  if (cfg.type === "mapping") {
    ...
    const ovalCY   = (topY + botY) / 2;
    const ovalH    = usable + 24;
    const ovalW    = 56;
    const ovalTop  = ovalCY - ovalH/2;
    const setLabelY = ovalTop - 18;
    ...
    // defs — color mode uses per-palette arrowhead markers; print mode uses
    // a single black arrowhead. Crossings are disambiguated by alternating
    // bezier curvature (see arrow render below), not by dash patterns —
    // Susanna Epp uses solid arrows throughout, dashed/dotted are not
    // standard mathematical notation in function diagrams.
    const palette = ["#3b82f6","#ef4444","#8b5cf6","#f59e0b","#10b981","#ec4899"];
    const printMode = !!cfg.printMode;
    const defs = mk("defs", {});
    if (printMode) {
      const marker = mk("marker", { id:"arrowhead-bw", markerWidth:"8", markerHeight:"6",
        refX:"8", refY:"3", orient:"auto" });
      marker.appendChild(mk("polygon", { points:"0 0, 8 3, 0 6", fill:"#000000" }));
    ...
    function renderOneMapping(mc, x0, x1) {
      const innerW = x1 - x0;
      const colLeft  = x0 + innerW * 0.28;
      const colRight = x0 + innerW * 0.72;

      const domain   = mc.domain   || [];
      const codomain = mc.codomain || [];
      const arrows   = mc.arrows   || [];
      ...
      [colLeft, colRight].forEach(cx => {
        svg.appendChild(mk("ellipse", { cx: String(cx), cy: String(ovalCY),
          rx: String(ovalW/2), ry: String(ovalH/2),
          fill:"#f1f5f9", stroke:"#94a3b8", "stroke-width":"2.8" }));
      });

      // set labels — sit ≥18px above the oval's top edge
      if (mc.domainLabel || mc.codomainLabel) {
        const dl = mk("text", { x: String(colLeft), y: String(setLabelY),
          "text-anchor":"middle", "font-size":"20", "font-family":"sans-serif",
          fill:"#475569", "font-style":"italic", "font-weight":"bold" });
        dl.textContent = mc.domainLabel || "A";
        svg.appendChild(dl);

        const cl = mk("text", { x: String(colRight), y: String(setLabelY),
          "text-anchor":"middle", "font-size":"20", "font-family":"sans-serif",
          fill:"#475569", "font-style":"italic", "font-weight":"bold" });
        cl.textContent = mc.codomainLabel || "B";
        svg.appendChild(cl);
      }

      // function label (e.g. "f", "g") — centered above the arrows between ovals
      if (mc.functionLabel) {
        const fl = mk("text", {
          x: String((colLeft + colRight) / 2), y: String(setLabelY),
          "text-anchor":"middle", "font-size":"18", "font-family":"sans-serif",
          fill:"#1e293b", "font-style":"italic" });
        fl.textContent = mc.functionLabel;
        svg.appendChild(fl);
      }
      ...
      domain.forEach((label, i) => {
        ...
        const t = mk("text", { x: String(colLeft), y: String(cy + 4),
          "text-anchor":"middle", "font-size":"16", "font-family":"sans-serif", fill:"#1e293b" });
        t.textContent = label;
        svg.appendChild(t);
      });
      ...
      // arrows — solid bezier curves. The control point is offset perpendicular
      // to the chord by ±6% of the horizontal span, alternating sign per source
      // index, so two arrows ending at the same codomain element approach from
      // visually distinct angles. Used in both color and B&W modes.
      arrows.forEach(([from, to]) => {
        const x1c = colLeft  + 15;
        const y1c = domY(from);
        const x2c = colRight - 15;
        const y2c = codY(to);
        const midX = (x1c + x2c) / 2;
        const midY = (y1c + y2c) / 2;
        const perpOffset = ((from % 2 === 0) ? 1 : -1) * (Math.abs(x2c - x1c) * 0.06);
        const dx = x2c - x1c, dy = y2c - y1c;
        const len = Math.sqrt(dx*dx + dy*dy) || 1;
        const cpx = midX + (-dy / len) * perpOffset;
        const cpy = midY + ( dx / len) * perpOffset;
        const colorIdx = from % palette.length;
        const stroke = printMode ? "#000000" : palette[colorIdx];
        const markerEnd = printMode ? "url(#arrowhead-bw)" : `url(#arrowhead-${colorIdx})`;
        svg.appendChild(mk("path", {
          d: `M ${x1c} ${y1c} Q ${cpx} ${cpy} ${x2c} ${y2c}`,
          stroke,
          "stroke-width": "2",
          fill: "none",
    ...
    if (cfg.second) {
      ...
      renderOneMapping(cfg, 0, halfW);
      renderOneMapping(cfg.second, halfW + gap, width);
    } else {
      renderOneMapping(cfg, 0, width);
    }
  ...
  if (cfg.type === "venn") {
    ...
    const shade = (fill, clipPathId, opacity = 0.35) => {
      const rect = mk("rect", {
    ...
    // Region shading is COLOR MODE ONLY. Print mode renders clean black
    // outlines with no fill — no patterns, no gray, no shading at all —
    // so the diagram reads like a textbook figure (white background, black
    // outlines, plain black element labels).
    if (!printMode) {
      if (shaded.includes("AorB") || shaded.includes("A") || shaded.includes("AnotB")) {
        shade(colorA, "clipA", 0.35);
      }
      if (shaded.includes("AorB") || shaded.includes("B") || shaded.includes("BnotA")) {
        shade(colorB, "clipB", 0.35);
      }
    ...
    svg.appendChild(mk("circle", {
      cx: String(cA.x), cy: String(cA.y), r: String(r),
      fill: "none", stroke: printMode ? "#000000" : colorA, "stroke-width": "2.8"
    }));
    svg.appendChild(mk("circle", {
      cx: String(cB.x), cy: String(cB.y), r: String(r),
      fill: "none", stroke: printMode ? "#000000" : colorB, "stroke-width": "2.8"
    }));

    svg.appendChild(mk("text", {
      x: String(cA.x - r * 0.7), y: String(cA.y - r - 14),
      "text-anchor": "middle", "font-size": "20", "font-family": "sans-serif",
      fill: printMode ? "#000000" : colorA, "font-weight": "bold", "font-style": "italic"
    }, labelA));
    svg.appendChild(mk("text", {
      x: String(cB.x + r * 0.7), y: String(cB.y - r - 14),
      "text-anchor": "middle", "font-size": "20", "font-family": "sans-serif",
      fill: printMode ? "#000000" : colorB, "font-weight": "bold", "font-style": "italic"
    }, labelB));

    // Element labels — plain text in both modes. Print mode regions are
    // unfilled (clean black outlines only), so the labels sit on the white
    // background with no halo needed.
    const elementLabelAttrs = (x, y) => ({
      x: String(x), y: String(y),
      "text-anchor": "middle", "font-size": "16", "font-family": "sans-serif",
      fill: printMode ? "#000000" : "#1e293b",
    });
    (cfg.elementsA || []).forEach((el, i) => {
      svg.appendChild(mk(
        "text",
        elementLabelAttrs(cA.x - r * 0.55, cy - 10 + i * 20),
        el
      ));
    });
    (cfg.elementsB || []).forEach((el, i) => {
      svg.appendChild(mk(
        "text",
        elementLabelAttrs(cB.x + r * 0.55, cy - 10 + i * 20),
        el
      ));
    });
    (cfg.elementsAB || []).forEach((el, i) => {
      svg.appendChild(mk(
        "text",
        elementLabelAttrs(cx, cy - 10 + i * 20),
        el
      ));
    });
  ...
  if (!yDom) {
    ...
    if (cfg.gPieces) cfg.gPieces.forEach(p => fns.push(p.fn)); // §2.3 functionPair g-family
    if (fns.length) {
  ...
  // Arrowhead marker for "extends to infinity" indicators on piecewise pieces.
  // orient="auto-start-reverse" lets one marker serve both marker-start
  // (reversed automatically) and marker-end.
  defs.append("marker")
    .attr("id", "fn-arrow")
    ...
    .append("path").attr("d", "M 0 0 L 10 5 L 0 10 z").attr("fill", COL.blue);

  // Smart tick generator — max ~10 ticks, snapped to "nice numbers"
  // step ladder of {1, 2, 2.5, 5} × 10^n. This matches Excel/Matlab/D3 nice
  // numbers convention and prevents the old algorithm's bug where ranges
  // like 8 produced step=0.8 (giving 0.7, 1.4, 2.1, ... tick labels).
  // Mirrors the niceStep/niceTicks pair at line ~1099 of this file.
  function smartTicks(min, max, maxTicks=10) {
    ...
    const step = mult * mag;
    // Step in integer space to avoid floating-point drift, then divide back.
    const inv = 1 / step;
    const start = Math.ceil(min * inv) / inv;
    const ticks = [];
    for (let t = start; t <= max + step * 0.001; t = Math.round((t + step) * 1000) / 1000) {
      ticks.push(Math.round(t * 1000) / 1000);
    }
    return ticks;
  }

  // Explicit fixed-step ticks across [min, max] (every multiple of `step`). Used
  // only when a config supplies an axis tick step (e.g. §2.2 wants integer x
  // labels across a wide domain that smartTicks would thin to even numbers).
  // Returns null for a non-positive / unsafe step so callers fall back to smartTicks.
  function stepTicks(min, max, step) {
    if (!(typeof step === "number" && step > 0) || (max - min) / step > 1000) return null;
    const out = [];
  ...
  // Per-axis tick values: honor cfg.xTickStep / cfg.yTickStep when present,
  // otherwise smartTicks (byte-identical to today's behavior when absent).
  const xTickVals = () => stepTicks(xDom[0], xDom[1], cfg.xTickStep) || smartTicks(xDom[0], xDom[1]);
  const yTickVals = () => stepTicks(yDom[0], yDom[1], cfg.yTickStep) || smartTicks(yDom[0], yDom[1]);
  ...
  function drawAxisNumbers() {
    ...
    xTickVals().filter(t => t !== 0).forEach(t => {
    ...
    yTickVals().filter(t => t !== 0).forEach(t => {
  ...
  function drawCurve(fnExpr, domainOverride, color, dashed, options = {}) {
    const [x0, x1] = domainOverride || xDom;
    const step = (x1 - x0) / 300;
    // Sample WITHOUT pre-filtering so d3.line().defined() can detect path breaks.
    // Filtering here would lose the break-points needed for asymptote detection.
    const pts = d3.range(x0, x1 + step * 0.5, step).map(x => [x, evalFn(fnExpr, x)]);
    // Asymptote / domain-edge / out-of-range detection. Mark points as
    // "undefined" so the d3 path breaks there instead of drawing a line across:
    //   1. Non-finite y (NaN / Infinity) — domain edge of sqrt(x<0), log(x<=0)
    //   2. y far outside yDom (more than yRange beyond either end) — asymptote tails
    //   3. Adjacent samples whose y values jump by more than yRange — sign-flip
    //      across an asymptote (e.g. 1/x going from -1e15 to +1e15 near x=0)
    const yRange = yDom[1] - yDom[0];
    const ok = new Array(pts.length);
    for (let i = 0; i < pts.length; i++) {
      const y = pts[i][1];
      ok[i] = isFinite(y) && y >= yDom[0] - yRange && y <= yDom[1] + yRange;
    }
    for (let i = 1; i < pts.length; i++) {
      if (ok[i] && ok[i - 1] && Math.abs(pts[i][1] - pts[i - 1][1]) > yRange) {
        ok[i] = false;
      }
    }
    if (!ok.some(Boolean)) {
      if (typeof window !== "undefined" && window.console) {
        console.warn("[graphRendering.drawCurve] No finite points produced for fn:", fnExpr, "over domain", [x0, x1]);
      }
      return;
    }
    const line = d3.line()
      ...
      .y(d => yScale(d[1]))
      .defined((d, i) => ok[i]);
    const path = gClip.append("path").datum(pts).attr("d", line)
      .attr("fill", "none").attr("stroke", color || COL.blue).attr("stroke-width", 2.5)
      .attr("stroke-dasharray", dashed ? "6,4" : "none");
    if (options.extendsLeft) path.attr("marker-start", "url(#fn-arrow)");
    if (options.extendsRight) path.attr("marker-end", "url(#fn-arrow)");
  }
  ...
  // §2.2 limit graphs: dashed, muted vertical-asymptote lines for any x in
  // cfg.verticalAsymptotes (numbers). Mirrors the stat-chart drawBoundary style
  // (dashed 5,4, xScale for the pixel-x) but spans the full plot height and
  // labels "x = {value}" near the top. No-op when the field is absent/empty, so
  // every existing graph renders exactly as before.
  function drawVerticalAsymptotes() {
    if (!Array.isArray(cfg.verticalAsymptotes)) return;
    cfg.verticalAsymptotes.forEach(vx => {
      if (typeof vx !== "number" || !isFinite(vx)) return;
      if (vx < xDom[0] || vx > xDom[1]) return; // skip outside the current xDomain
      const px = xScale(vx);
      g.append("line").attr("x1", px).attr("x2", px).attr("y1", 0).attr("y2", iH)
        .attr("stroke", COL.muted).attr("stroke-width", 1.5).attr("stroke-dasharray", "5,4");
      const rounded = Math.round(vx * 100) / 100;
      g.append("text").attr("x", px + 4).attr("y", 12)
        .attr("fill", COL.muted).attr("font-size", 11).attr("font-family", "sans-serif")
        .text(`x = ${rounded}`);
    });
  }

  // §2.6 limits-at-infinity graphs: dashed, muted horizontal-asymptote lines for
  // any entry in cfg.horizontalAsymptotes. Each entry is { y, label } (a bare
  // number is tolerated: label = String(number)). Exact mirror of
  // drawVerticalAsymptotes — same dash pattern, stroke, opacity, and label font/
  // color — but spans the full plot WIDTH at the y-pixel for entry.y and labels
  // "y = {label}" at the right edge just above the line. No-op when the field is
  // absent/empty, so every existing graph renders exactly as before.
  function drawHorizontalAsymptotes() {
    if (!Array.isArray(cfg.horizontalAsymptotes)) return;
    cfg.horizontalAsymptotes.forEach(entry => {
      const hy = typeof entry === "number" ? entry : (entry && entry.y);
      if (typeof hy !== "number" || !isFinite(hy)) return;
      if (hy < yDom[0] || hy > yDom[1]) return; // skip outside the current yDomain
      const py = yScale(hy);
      g.append("line").attr("x1", 0).attr("x2", iW).attr("y1", py).attr("y2", py)
        .attr("stroke", COL.muted).attr("stroke-width", 1.5).attr("stroke-dasharray", "5,4");
      const label = typeof entry === "number" ? String(entry) : String(entry.label);
      g.append("text").attr("x", iW - 4).attr("y", py - 4).attr("text-anchor", "end")
        .attr("fill", COL.muted).attr("font-size", 11).attr("font-family", "sans-serif")
        .text(`y = ${label}`);
    });
  }
  ...
  // Grid + axes go UNDER the curves; axis NUMBER labels are drawn LAST (below,
  // after the curve branches) so a tick label that sits where a curve crosses
  // (e.g. y=2 near the y-axis) is no longer painted over by the curve stroke.
  drawGrid();
  drawAxes();
  ...
  if (cfg.type === "overlay") {
    // Overlay: two or more curves on the same axes. Used for §1.3-style
    // before/after figures (parent f and transformed h on one set of axes).
    //
    // cfg.fns: array of { fn, label?, color?, dashed? } entries.
    //   color is one of: "blue", "red", "green", "orange". Defaults rotate
    //   through the palette. Default for first curve is blue, second is red.
    //   dashed: true draws the curve as a dashed line (used for the parent).
    //   label: optional text label drawn at the right end of the curve.
    // cfg.holes / cfg.points: applied globally (rare for overlay).
    const overlayPalette = [COL.blue, COL.red, COL.green || "#10b981", "#f59e0b"];
    const colorMap = { blue: COL.blue, red: COL.red, green: COL.green || "#10b981", orange: "#f59e0b" };
    const fns = Array.isArray(cfg.fns) ? cfg.fns : [];
    fns.forEach((entry, i) => {
      if (!entry || !entry.fn) return;
      const color = colorMap[entry.color] || overlayPalette[i % overlayPalette.length];
      const dashed = entry.dashed === true;
      drawCurve(entry.fn, null, color, dashed);
      if (showLabel && entry.label) {
        const lx = labelX(entry.fn, true);
        const ly = evalFn(entry.fn, lx);
    ...
    (cfg.holes  || []).forEach(([x, y]) => drawOpenCircle(x, y, COL.text));
    (cfg.points || []).forEach(([x, y]) => drawFilledDot(x, y, COL.text));
    ...
    drawVerticalAsymptotes();
    drawHorizontalAsymptotes();

  } else if (cfg.type === "piecewise") {
    (cfg.pieces || []).forEach(p => drawCurve(p.fn, p.domain, COL.blue, false, {
      extendsLeft: p.extendsLeft === true,
      extendsRight: p.extendsRight === true,
    }));
    (cfg.holes  || []).forEach(([x, y]) => drawOpenCircle(x, y, COL.blue));
    (cfg.points || []).forEach(([x, y]) => drawFilledDot(x, y, COL.blue));
    ...
    // §2.3 functionPair: g's family (red) on the SAME axes + colored f/g labels.
    // Additive — a normal piecewise config has no gPieces, so this is a no-op.
    if (Array.isArray(cfg.gPieces) && cfg.gPieces.length) {
      cfg.gPieces.forEach(p => drawCurve(p.fn, p.domain, COL.red, false, {
        extendsLeft: p.extendsLeft === true,
        extendsRight: p.extendsRight === true,
      }));
      (cfg.gHoles  || []).forEach(([x, y]) => drawOpenCircle(x, y, COL.red));
      (cfg.gPoints || []).forEach(([x, y]) => drawFilledDot(x, y, COL.red));
      // Colored curve labels at distinct x-fractions so f and g don't overlap.
      const placeCurveLabel = (pieces, frac, color, text) => {
        if (!pieces || !pieces.length) return;
        const last = pieces[pieces.length - 1];
        let lx = xDom[0] + (xDom[1] - xDom[0]) * frac;
        if (Array.isArray(last.domain)) lx = Math.max(last.domain[0], Math.min(last.domain[1], lx));
        const ly = evalFn(last.fn, lx);
        if (!isFinite(ly) || ly < yDom[0] || ly > yDom[1]) return;
        g.append("text").attr("x", xScale(lx)).attr("y", yScale(ly) - 8)
          .attr("fill", color).attr("font-size", 13).attr("font-style", "italic")
          .attr("font-weight", "600").attr("font-family", "sans-serif").text(text);
      };
      placeCurveLabel(cfg.pieces, 0.30, COL.blue, cfg.fLabel || "f");
      placeCurveLabel(cfg.gPieces, 0.70, COL.red, cfg.gLabel || "g");
    }
    drawVerticalAsymptotes();
    drawHorizontalAsymptotes();
  ...
  // Axis number labels last, so they sit ON TOP of the curves (fixes y-tick
  // labels being occluded where a curve crosses near the y-axis).
  drawAxisNumbers();
...
export function renderStatChartToSVG(chartConfig, width=480, height=300) {
  ...
  const cfg = rawCfg.type === "standard_normal"
    ...
    : rawCfg;
  // Bottom margin grows on normal-dist charts so μ/σ labels (rendered at iH+58 and iH+70)
  // are not clipped at the chart bottom.
  const _isNormDist = cfg.type === "continuous_dist" &&
    (cfg.distType === "normal" || cfg.distType === "standard_normal" || !cfg.distType);
  const _showMuM    = _isNormDist && cfg.showMu    !== false;
  const _showSigmaM = _isNormDist && cfg.showSigma !== false;
  const _bottomM = _isNormDist
    ? 55 + (_showMuM ? 18 : 0) + (_showSigmaM ? 18 : 0)
    : 68;
  const margin = {top:30, right:30, bottom: _bottomM, left:55};
  const iW = width  - margin.left - margin.right;
  const iH = height - margin.top  - margin.bottom;
  ...
  // ── Continuous Probability Distribution (Normal / Uniform / Exponential) ──
    ...
    if ((distType === "normal" || distType === "standard_normal") && xScale(mu) >= 0 && xScale(mu) <= iW) {
      const showMu    = cfg.showMu    !== false;
      const showSigma = cfg.showSigma !== false;
      const sigmaVal  = isStdNorm ? 1 : sigma;
      if (showMu) {
      ...
      // \u03bc label \u2014 same logic as before, now gated by showMu
      if (showMu && sFrom !== mu && sTo !== mu)
        axisLabel(muLabel, xScale(mu), iH+58, "middle", 9);
      // \u03c3 label \u2014 same font/size/color/anchor as \u03bc, one line below.
      // 12px gap matches the line-height of the 9px font used for \u03bc.
      if (showSigma)
        axisLabel("\u03c3=" + sigmaVal, xScale(mu), iH+70, "middle", 9);
    }
