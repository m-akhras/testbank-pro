...
export function renderGraphToSVG(graphConfig, width = 480, height = 300) {
  ...
  // ── functionPair (§2.3 Limit Laws) ───────────────────────────────────────
  // Two independent piecewise graphs (f and g) side by side in one SVG. We RENDER
  // EACH PANEL by recursively calling this same function on its sub-config, then
  // nest the two returned SVGs at an x-offset (mirroring mapping+second). This
  // reuses the full-fidelity piecewise path UNCHANGED — single-panel output is
  // byte-identical — and keeps one graphConfig per question, so exports emit one
  // combined PNG. Note: each nested panel carries its own <defs>; clip ids are
  // randomized (safe), and the fixed "fn-arrow" marker id duplicates across panels
  // but is identical content, so arrowheads still resolve correctly.
  if (cfg.type === "functionPair") {
    ...
    parent.setAttribute("xmlns", svgNS);
    parent.setAttribute("width", String(width));
    parent.setAttribute("height", String(height));
    parent.setAttribute("viewBox", `0 0 ${width} ${height}`);

    const panels = [
      { sub: cfg.f, x: 0, label: cfg.fLabel || "f" },
      { sub: cfg.g, x: halfW + gap, label: cfg.gLabel || "g" },
    ];
    panels.forEach(({ sub, x, label }) => {
      // Suppress the per-curve "f(x)" label inside each panel (shallow copy — do
      // NOT mutate cfg.f/cfg.g): the big italic "f"/"g" panel labels already
      // identify the curves, and the single-panel default would otherwise
      // mislabel the g panel as "f(x)".
      const panelCfg = { ...(sub || {}), showFnLabel: false };
      const panelSvg = renderGraphToSVG(panelCfg, halfW, height);
      if (panelSvg) {
        // Nest as a sub-viewport at the panel's x-offset.
        panelSvg.setAttribute("x", String(x));
        panelSvg.setAttribute("y", "0");
        panelSvg.setAttribute("width", String(halfW));
        panelSvg.setAttribute("height", String(height));
        parent.appendChild(panelSvg);
      }
      // Panel label (top-left), drawn on the parent so it sits above the panel.
