...
import { mathToCanvasHTML } from "../math/html.js";
...
function _explanationFeedback(q, options = {}) {
  // Gate on options.includeExplanations (default false). When false, no feedback
  // is emitted regardless of whether the question has an explanation. Instructors
  // can still see explanations via the Word/Answer-Key export.
  if (!options.includeExplanations) return { inResp: "", afterResp: "" };
...
// Question types that QTI export can't handle today. Branched FR has no
// MCQ choices and Branched MCQ uses a stem + parts shape Canvas doesn't model.
// Returns true when the question SHOULD be exported, false when it should
// be silently skipped from the QTI bundle. Branched MCQ also emits a one-time
// console.warn so the user notices their choices weren't pushed to Canvas.
const _warnedBranchedMCQ = new Set();
function _isQTIExportable(q) {
  if (!q) return false;
  if (q.type === "Branched Free Response") return false;
  if (q.type === "Branched MCQ") {
    const k = q.id || `${q.section}|${q.stem || q.question}`;
    if (!_warnedBranchedMCQ.has(k)) {
      _warnedBranchedMCQ.add(k);
      console.warn("Branched MCQ QTI export not yet implemented — skipping question", k);
    }
    return false;
  }
  return true;
...
async function _choiceGraphConfigToPng(cfg) {
  if (!cfg) return "";
  try {
    // Specialized component types (vectorField, contour, region, parametric, surface, path)
    // use cfg.graphType and have their own dedicated rasterizers.
    ...
    // Standard function-graph types (single, overlay, piecewise) and stat charts.
    // Mirrors the stem-graph rasterization cascade in buildQTIZip (~lines 351-362)
    // so per-choice graphs render the same way stem graphs do, just at choice size
    // (300x280 vs the stem's 480x280). Previously these all fell through to "" and
    // every Canvas choice slot got an empty src.
    if (isNewGraphConfig(cfg)) {
      return await newGraphConfigToPng(cfg, 300, 280) || "";
    }
    const _isStatQTI = cfg.type &&
      ["bar","histogram","scatter","discrete_dist","continuous_dist","standard_normal"].includes(cfg.type);
    if (_isStatQTI) {
      return await statChartToBase64PNG(canvasExportConfig(cfg), 300, 280) || "";
    }
    return await graphToBase64PNG(canvasExportConfig(cfg), 300, 280) || "";
  } catch (_e) {
    return "";
  }
}
...
// Returns -1 when no choice matches the answer (the condition the four
// builders guard against — see answerMatchesAChoice and the per-item guards).
export function _findCorrectChoiceIdx(q) {
...
// True when the question's correct answer can be resolved to one of its
// choices using the SAME lenient matcher the QTI builders use, so callers
// never warn about a question that would actually export fine. Non-MC types
// and choice-less questions are out of scope (return true; handled elsewhere).
export function answerMatchesAChoice(q) {
  if (q.type !== "Multiple Choice" && q.type !== "True/False") return true;
  if (!q.choices || q.choices.length === 0) return true; // missing-choices handled separately
  return _findCorrectChoiceIdx(q) >= 0;
}

// BLOCKING guard input: collect the Multiple Choice questions that would import
// into Canvas WITHOUT a correct answer marked — i.e. the answer matches none of
// the choices under _findCorrectChoiceIdx (the SAME lenient matcher the writers
// use to key the item). These are exactly the items Canvas rejects with
// "couldn't determine the correct answers".
//
// Scope is deliberately Multiple-Choice-only: the QTI writers only key items
// where `q.type === "Multiple Choice" && q.choices` (see makeItem in each
// builder). True/False is emitted via the short-answer path and is never keyed,
// so it can't hit the findIndex -1 case; choice-less types (Formula, Free
// Response, Fill in the Blank, Branched) legitimately have an answer with no
// matching choice. Including any of those would block exports the writers
// produce just fine.
//
// `versionsToCheck` is a flat array of { label, questions }. Returns
// [{ label, qNum, section, answer }] so callers can name each offender.
export function collectUnkeyedMCQuestions(versionsToCheck) {
  const broken = [];
...
// Package choice graphs as files in web_resources/ and reference via $IMS-CC-FILEBASE$,
// mirroring the stem-graph delivery pattern. Canvas's HTML sanitizer strips inline
// data: image URLs from imported quiz content, so choice graphs delivered as data
// URLs render as broken imgs. File-based references resolve correctly.
async function _resolveChoiceGraphs(xml, zip, webResources) {
  ...
  for (const m of matches) {
    ...
    if (!dataUrl) {
      out = out.split(`VFCHOICE_PH_${key}`).join("");
      continue;
    }
    // Decode the data: URL to bytes and add as a file in web_resources/.
    // Filename prefix "choicegraph_" prevents collision with stem graphs ("graph_").
    n++;
    const filename = `choicegraph_${n}.png`;
    const path = `web_resources/${filename}`;
    try {
      const b64 = dataUrl.replace(/^data:image\/png;base64,/, "");
      const bytes = Uint8Array.from(atob(b64), ch => ch.charCodeAt(0));
      zip.file(path, bytes);
      webResources.push({ path, identifier: `choicegraph_res_${n}` });
      out = out.split(`VFCHOICE_PH_${key}`).join(`$IMS-CC-FILEBASE$${path}`);
    } catch (_e) {
      // Decoding failed — fall back to empty src like the original behavior
      out = out.split(`VFCHOICE_PH_${key}`).join("");
    }
  }
  return out;
...
export function validateQTIExport(versionsToCheck) {
  ...
  versionsToCheck.forEach(v => {
    (v.questions || []).forEach((q, qi) => {
      ...
      // Answer-not-in-choices: uses the SAME lenient matcher the builders use,
      // so we never warn about a question that would actually export keyed.
      // Non-blocking — the export proceeds and the item imports unkeyed.
      if (!answerMatchesAChoice(q)) {
        warnings.push(`${v.label} Q${qi+1}: correct answer "${q.answer}" is not among the choices — it will import WITHOUT a correct answer marked. Fix the question or set the answer in Canvas.`);
      }
    });
  });
  return warnings;
...
// Strip title/probability from graphConfig for Canvas export unless flagged to include
export function buildQTI(questions, course, vLabel, useGroups=false, pointsPerQ=1, includeExplanations=false) {
  const canvasQ = questions.filter(_isQTIExportable);
  ...
  function makeItem(q, id, num) {
    ...
    const fb = _explanationFeedback(q, { includeExplanations });
    if (isMC) {
      ...
      // When the answer matches no choice (correct < 0), omit the answer-key
      // markup entirely rather than emit a negative ident Canvas rejects with
      // "couldn't determine the correct answers". The item still imports — the
      // stem and all choices are emitted — just with no correct answer marked.
      const correctBlock = correct >= 0
...
<quiz identifier="${assessmentId}" xmlns="http://canvas.instructure.com/xsd/cccv1p0">
  ...
  // Choice-graph placeholders: rasterize, package as web_resources files,
  // and rewrite to $IMS-CC-FILEBASE$ refs (same delivery pattern as stems).
  qtiXml = await _resolveChoiceGraphs(qtiXml, zip, webResources);
...
export async function buildClassroomSectionsQTI(classSectionVersions, course, useGroups=true, pointsPerQ=1, includeExplanations=false) {
  if (!_JSZip()) {
    await new Promise((res, rej) => {
      const s = document.createElement("script");
      s.src = "https://cdnjs.cloudflare.com/ajax/libs/jszip/3.10.1/jszip.min.js";
  ...
  function makeItem(q, ident, qnum) {
    ...
    if (isMC) {
      ...
      const correct = _findCorrectChoiceIdx(q);
      // Omit the answer key (rather than emit a negative ident) when the answer
      // matches no choice — the item imports unkeyed instead of breaking Canvas.
      const correctBlock = correct >= 0
        ? `<respcondition continue="No"><conditionvar><varequal respident="response1">${ident}_${correct}</varequal></conditionvar><setvar action="Set" varname="SCORE">100</setvar></respcondition>`
        : "";
...
<manifest identifier="${assessId}_manifest" xmlns="http://www.imsglobal.org/xsd/imsccv1p1/imscp_v1p1" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.imsglobal.org/xsd/imsccv1p1/imscp_v1p1 http://www.imsglobal.org/xsd/imsccv1p1/imscp_v1p1.xsd">
  ...
  <resources>
    ...
    // Choice-graph placeholders: rasterize, package as web_resources files,
    // and rewrite to $IMS-CC-FILEBASE$ refs (same delivery pattern as stems).
    resolvedQtiXml = await _resolveChoiceGraphs(resolvedQtiXml, zip, secWebResources);
...
export function buildQTICompare(versions, course, useGroups=false, pointsPerQ=1, includeExplanations=false) {
...
export function buildQTIAllSectionsMerged(classSectionVersions, course, pointsPerQ=1, includeExplanations=false) {
  function uid8() { return Math.random().toString(16).slice(2,10).padEnd(8,'0'); }
  ...
  for (let qi = 0; qi < numQ; qi++) {
    ...
    const totalItems = sortedSecs.reduce((sum, sec) =>
      sum + (classSectionVersions[sec]||[]).filter(v => _isQTIExportable(v.questions[qi])).length, 0);
    groupSections.push(`  <section ident="${sectionId}">
    <selection_ordering>
      <selection>
        <selection_number>1</selection_number>
