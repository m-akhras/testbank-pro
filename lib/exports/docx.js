...
// Inline answer block (only used when includeAnswers=true). Visually distinct:
// thick green left border, light green fill, "✓ Answer:" bold-green prefix
// followed by the answer (rendered as inline OMML), then any solution steps.
function _qAnswerBlock(answer, explanation) {
  const labelRpr = `<w:rPr><w:b/><w:sz w:val="22"/><w:color w:val="1A7A4A"/></w:rPr>`;

  let answerRuns = "";
  if (answer != null && String(answer).length > 0) {
    const raw = String(answer).replace(/ /g, " ");
    try {
      answerRuns = mathToOmml(raw);
    } catch (e) {
      const safe = raw.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
      answerRuns = `<w:r><w:rPr><w:sz w:val="22"/></w:rPr><w:t xml:space="preserve">${safe}</w:t></w:r>`;
    }
  }

  const headlinePpr = `<w:pPr><w:spacing w:after="60"/></w:pPr>`;
  let cellContent = `<w:p>${headlinePpr}<w:r>${labelRpr}<w:t xml:space="preserve">✓ Answer: </w:t></w:r>${answerRuns}</w:p>`;
  ...
  if (explanation) {
    ...
    steps.forEach((step, i) => {
      ...
      try {
        ...
        cellContent += `<w:p>${stepPpr}<w:r><w:rPr><w:sz w:val="20"/><w:color w:val="444444"/></w:rPr><w:t xml:space="preserve">${safe}</w:t></w:r></w:p>`;
      }
    });
  }
  ...
  return `<w:tbl>
    <w:tblPr>
      <w:tblW w:w="11088" w:type="dxa"/>
      <w:tblBorders>
        <w:top w:val="single" w:sz="4" w:color="1A7A4A"/>
        <w:left w:val="single" w:sz="24" w:color="1A7A4A"/>
        <w:bottom w:val="single" w:sz="4" w:color="1A7A4A"/>
        <w:right w:val="single" w:sz="4" w:color="C0E5D0"/>
        <w:insideH w:val="nil"/>
        <w:insideV w:val="nil"/>
      </w:tblBorders>
      <w:tblLook w:val="04A0"/>
    ...
    <w:tr>
      <w:tc>
        <w:tcPr>
          <w:tcW w:w="11088" w:type="dxa"/>
          <w:shd w:val="clear" w:color="auto" w:fill="EAFBF1"/>
          <w:tcMar><w:top w:w="120" w:type="dxa"/><w:bottom w:w="120" w:type="dxa"/><w:left w:w="240" w:type="dxa"/><w:right w:w="200" w:type="dxa"/></w:tcMar>
        </w:tcPr>
        ${cellContent}
      </w:tc>
    </w:tr>
...
async function _renderQuestionsBody(questions, startNum, tc, includeAnswers) {
  ...
  for (let i = 0; i < questions.length; i++) {
    ...
    if (q.type === "Branched") {
      // Branched questions: stem + parts. Each part gets its own answer area
      // (per the same rules), since each part is independently answered.
      const hdr = _qBuildHeader(num, q.stem || "", qMarks);
      body += hdr.xml;
      if (!hdr.inlined) body += _qMathPara(q.stem || "");

      const parts = q.parts || [];
      const partMarks = Math.max(1, Math.floor(qMarks / Math.max(1, parts.length)));

      parts.forEach((p, pi) => {
        body += _qPara(`(${String.fromCharCode(97+pi)})`, {indent:360, size:22, spacing:60});
        body += _qMathPara(p.question || "", {indent:360});

        const partIsMCQ = Array.isArray(p.choices) && p.choices.length > 0;
        if (partIsMCQ) {
          p.choices.forEach((c,ci) => {
            body += _qMathPara(`${String.fromCharCode(65+ci)}. ${c}`, {indent:720});
          });
        }

        // Answer area (the ONLY thing that differs between paths)
        if (includeAnswers) {
          if (p.answer != null && String(p.answer).length > 0) {
            body += _qAnswerBlock(p.answer, p.explanation);
          }
        } else if (!partIsMCQ) {
          body += _qBlankSpace(partMarks);
        }
      });
    } else {
      ...
      if (_isMCQ(q)) {
        q.choices.forEach((c,ci) => {
          body += _qMathPara(`${String.fromCharCode(65+ci)}. ${c}`, {indent:360});
        });
      }
      ...
      if (includeAnswers) {
        if (q.answer != null && String(q.answer).length > 0) {
          body += _qAnswerBlock(q.answer, q.explanation);
        }
      } else if (!_isMCQ(q)) {
...
// Backwards-compatible entrypoint for the "Answer Key (.docx)" button. Produces
// a document with the SAME layout as buildDocx — same cover, header, footer,
// question order — but with includeAnswers=true so inline ✓ Answer blocks
// replace the blank student space. For multiple versions, each version's full
// exam (with inline answers) is concatenated into one document, separated by
// page breaks; cover/header/footer are shared.
export async function buildAnswerKey(versions, course, templateConfig=null) {
  if (!versions || versions.length === 0) return null;

  if (versions.length === 1) {
    const v = versions[0];
    const cs = v.questions[0]?.classSection ?? null;
    return await buildDocx(v.questions, course, v.label, cs, 1, templateConfig, true);
  }
  ...
  for (let vi = 0; vi < versions.length; vi++) {
    const v = versions[vi];
    const cs = v.questions[0]?.classSection ?? null;
    const titleLabel = cs ? `Section ${cs} — Version ${v.label}` : `Exam Version ${v.label}`;
    if (vi > 0) {
      // Page break between version sections (NOT for an answer key — both halves
      // already contain inline answers; this just separates two full exams).
      body += `<w:p><w:r><w:br w:type="page"/></w:r></w:p>`;
    }
    body += _qPara(`${course} — ${titleLabel}`, {bold:true, size:32, spacing:120});
    body += await _renderQuestionsBody(v.questions, 1, tc, true);
    totalQuestionCount = Math.max(totalQuestionCount, v.questions.length);
  }

  const firstCs = versions[0]?.questions[0]?.classSection ?? null;
  return await _assembleDocxBlob({
    body,
    course,
    classSection: firstCs,
    tc,
    logoData,
