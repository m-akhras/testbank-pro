...
export async function buildDocx(questions, course, vLabel, classSection=null, startNum=1, templateConfig=null, includeAnswers=false) {
  ...
  function buildQuestionHeader(num, text, marks) {
    ...
    let inlined = true;
    if (raw && !hasPipeTable) {
      // Split on common expression separators (commas, semicolons, " and ", " or ").
      // Each non-separator segment renders as its own atomic <m:oMath> so Word can
      // wrap BETWEEN expressions but not WITHIN a single equation. Internal spaces
      // inside each math segment are also replaced with NBSP as belt-and-suspenders
      // for the plain-text fallback path.
      const segments = normalized.split(/(,\s*|\s*;\s*|\s+and\s+|\s+or\s+)/i);
      const parts = segments.map((seg, idx) => {
        if (!seg) return "";
        if (idx % 2 === 1) {
          // Separator — plain text run, regular spaces (lets the line break here)
          const safe = seg.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
          return `<w:r>${textRpr}<w:t xml:space="preserve">${safe}</w:t></w:r>`;
        }
        // Math segment — NBSP internal spaces, then render as inline OMML
  ...
  for (let i = 0; i < questions.length; i++) {
    ...
    if (q.type === "Branched") {
      ...
      (q.parts||[]).forEach((p, pi) => {
        ...
        if (includeAnswers && p.answer) body += para(`Answer: ${p.answer}`, {indent:360, size:22, color:"1a7a4a", spacing:80});
      });
    } else {
      ...
      if (includeAnswers && q.answer) body += mathPara(`✓ Answer: ${q.answer}`, {indent:0, size:22, color:"1a7a4a", spacing:60});
      if (includeAnswers && q.explanation) body += mathPara(`Note: ${q.explanation}`, {indent:0, size:20, color:"666666", spacing:120});
    }
