...
// ── Prose/math segmentation for stems ────────────────────────────────────────
// English prose words rendered as upright text runs; everything else is treated
// as an atomic inline math island so Word wraps at prose word-boundaries and
// never splits a function mid-expression. Conservative — function names like
// cos/sin/ln/lim/exp are intentionally NOT here (they are math).
const PROSE_WORDS = new Set([
  "let","find","solve","evaluate","determine","calculate","compute","simplify","given",
  "where","for","if","then","and","or","of","the","to","is","are","show","prove","consider",
  "suppose","assume","use","using","when","which","what","value","values","function","graph",
  "above","below","following","express","write","state","verify","hence","round","nearest",
  "decimal","place","places","in","on","at","with","from","that","this","all","each","both",
  "between","such","has","have","will","would","can","must","also","otherwise","respectively",
]);

// Split `text` into an ordered array of [{type:'prose'|'math', text}] segments.
// A token is prose ONLY if it is pure letters (len>=2, optional single trailing
// punctuation) AND a member of PROSE_WORDS; anything containing a math character
// is always math. A space is "math glue" (kept inside the math island) only when
// both neighbors are math; otherwise it is a breakable prose space. Consecutive
// same-class pieces (joined by their glue) coalesce into one segment.
// Multi-word math PHRASES that must stay whole — the word-by-word classifier
// below would otherwise shatter them on connectives ("from"/"to"/"of") or spaces,
// so they'd never reach mathToOmml as one segment. Each is anchored on a leading
// keyword (sum/product/integral) or " if " inside braces, so ordinary prose
// ("ranging from 1 to 10") and ordinary braces ("the set {1, 2, 3}") never match.
const ATOMIC_MATH_PHRASES = [
  // piecewise: a brace containing " if " (set-builder / Laplace lack " if ")
  /\{[^{}]*\sif\s[^{}]*\}/g,
  // summation / product, with an optional single-chunk "of <expr>" body
  /\b(?:sum|product|prod(?:uct)?)\s+from\s+[a-z]\s*=\s*\S+\s+to\s+\S+(?:\s+of\s+\S+)?/gi,
  // definite integral "... of <expr> d<var>" (the d<var> bounds the body)
  /\bintegral\s+from\s+\S+\s+to\s+\S+\s+of\s+.+?\s+d[a-z]\b/gi,
];

export function _segmentProseMath(text) {
  const str = String(text ?? "");
  if (!str) return [];

  // Pre-pass: carve out atomic math phrases as whole MATH segments before the
  // word-by-word pass runs on the remaining prose.
  const ranges = [];
  for (const re of ATOMIC_MATH_PHRASES) {
    re.lastIndex = 0;
    let m;
    while ((m = re.exec(str))) {
  ...
  if (ranges.length) {
    ...
    const push = (segs) => {
      for (const sg of segs) {
        ...
        if (last && last.type === sg.type) last.text += sg.text;
        else out.push(sg);
      }
    };
    let pos = 0;
    for (const [a, b] of merged) {
      if (a > pos) push(_classifyWords(str.slice(pos, a)));
      push([{ type: "math", text: str.slice(a, b) }]);
      pos = b;
    }
    if (pos < str.length) push(_classifyWords(str.slice(pos)));
    return _peelMathPunct(out);
  }
  return _peelMathPunct(_classifyWords(str));
}

// Word-by-word prose/math classification (the original algorithm).
function _classifyWords(str) {
  const isSpace = (t) => /^\s+$/.test(t);
  const classifyWord = (t) => {
    if (/^[A-Za-z]{2,}[.,;:?!]?$/.test(t)) {
      const core = t.replace(/[.,;:?!]$/, "").toLowerCase();
      if (PROSE_WORDS.has(core)) return "prose";
    }
    return "math";
  };

  // tokens with whitespace preserved as its own tokens
  const items = str.split(/(\s+)/).filter((t) => t !== "").map((t) =>
    isSpace(t) ? { kind: "space", text: t } : { kind: classifyWord(t), text: t }
  );

  // A space is math glue iff both neighbors are math.
  for (let i = 0; i < items.length; i++) {
    if (items[i].kind !== "space") continue;
    const prev = items[i - 1];
    const next = items[i + 1];
    items[i].kind = prev && prev.kind === "math" && next && next.kind === "math"
      ? "math" : "prose";
  }

  const segments = [];
  for (const it of items) {
    const type = it.kind === "math" ? "math" : "prose";
    const last = segments[segments.length - 1];
    if (last && last.type === type) last.text += it.text;
    else segments.push({ type, text: it.text });
  }
  return segments;
}

// Peel a single sentence-final punctuation char off the END of a math segment
// into the following prose run (or a new trailing prose segment) so it renders
// upright with normal spacing. Decimal-safe: a decimal dot is always followed
// by a digit, so it can never be the segment-final char and is never peeled.
function _peelMathPunct(segments) {
  for (let i = 0; i < segments.length; i++) {
    const seg = segments[i];
    if (seg.type !== "math" || seg.text.length < 2) continue;
    const ch = seg.text[seg.text.length - 1];
    if (!/[.,;:?!]/.test(ch)) continue;
    seg.text = seg.text.slice(0, -1);
    const next = segments[i + 1];
    if (next && next.type === "prose") {
      next.text = ch + next.text;
    } else {
      segments.splice(i + 1, 0, { type: "prose", text: ch });
...
// Build the run XML (no enclosing <w:p>) for a stem: prose → upright text run,
// math → inline <m:oMath> via mathToOmml. On a mathToOmml throw, that segment
// falls back to an escaped text run.
function _segmentsToRuns(text, textRpr) {
  const esc = (s) => s.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
  return _segmentProseMath(text).map((seg) => {
    if (seg.type === "prose") {
      return `<w:r>${textRpr}<w:t xml:space="preserve">${esc(seg.text)}</w:t></w:r>`;
    }
    try {
      return mathToOmml(seg.text);
    } catch (e) {
      return `<w:r>${textRpr}<w:t xml:space="preserve">${esc(seg.text)}</w:t></w:r>`;
    }
  }).join("");
...
// Emit ONE <w:p> (using the caller's ppr) whose prose is upright text runs and
// whose math is atomic inline <m:oMath>. textRpr keeps prose at the math size so
// the prose↔math seam is invisible.
export function _emitSegmentedParagraph(text, ppr, textRpr = `<w:rPr><w:sz w:val="22"/></w:rPr>`) {
  return `<w:p>${ppr}${_segmentsToRuns(text, textRpr)}</w:p>`;
}

// Emit a choice paragraph: "A." rendered as an upright prose run (forces the
// paragraph to be text, not all-math, so Word left-aligns it), a space, then
// the choice body as segmented prose+math.
export function _emitChoiceParagraph(letter, bodyText, ppr,
    textRpr = `<w:rPr><w:sz w:val="22"/></w:rPr>`) {
  const esc = (s) => s.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;");
  const label = `<w:r>${textRpr}<w:t xml:space="preserve">${esc(letter)}. </w:t></w:r>`;
  return `<w:p>${ppr}${label}${_segmentsToRuns(String(bodyText), textRpr)}</w:p>`;
}
...
function _qMathPara(text, opts={}) {
  ...
  return _emitSegmentedParagraph(normalized, ppr, `<w:rPr><w:sz w:val="22"/></w:rPr>`);
}
...
function _qBuildHeader(num, text, marks) {
  ...
  let inlined = true;
  if (raw && !hasPipeTable) {
    inlineContent = _segmentsToRuns(normalized, textRpr);
...
function _qAnswerBlock(answer, explanation) {
  ...
  // Answer: prose as upright text runs, math as atomic inline <m:oMath>.
  if (answer != null && String(answer).length > 0) {
    const ansPpr = `<w:pPr><w:keepLines/><w:spacing w:after="60"/><w:ind w:left="280"/></w:pPr>`;
    cellContent += _emitSegmentedParagraph(
      String(answer).replace(/\u00a0/g, " "), ansPpr, `<w:rPr><w:sz w:val="22"/></w:rPr>`
    );
  }
  ...
  if (explanation) {
    ...
    steps.forEach((step, i) => {
      ...
      cellContent += _emitSegmentedParagraph(String(step), stepPpr, `<w:rPr><w:sz w:val="22"/></w:rPr>`);
    });
  }
...
async function _renderQuestionsBody(questions, startNum, tc, includeAnswers) {
  ...
  for (let i = 0; i < questions.length; i++) {
      ...
      if (q.type === "Branched Free Response" && !includeAnswers) {
        ...
        try {
          ...
          if (isNewGraphConfig(q.graphConfig)) {
            ...
            const isDiscrete = q.course === "Discrete Mathematics";
            const cfgForExport = isDiscrete ? { ...q.graphConfig, printMode: true } : q.graphConfig;
            b64 = _isStat ? await statChartToBase64PNG(cfgForExport, 480, 280) : await graphToBase64PNG(cfgForExport, 480, 280);
          }
    ...
    if (q.type === "Branched MCQ") {
      ...
      const hdr = _qBuildHeader(num, stem, qMarks);
      body += hdr.xml;
      if (!hdr.inlined) body += _qMathPara(stem);

      const parts = Array.isArray(q.parts) ? q.parts : [];
      parts.forEach((p, pi) => {
        const partLetter = String.fromCharCode(97 + pi);
        const marksStr = (p && p.marks != null && String(p.marks).trim() !== "")
          ? ` [${String(p.marks).trim()} marks]` : "";
        body += _qMathPara(`(${partLetter})${marksStr} ${p?.question || ""}`, { spacing: 120 });

        const choices = Array.isArray(p?.choices) ? p.choices : [];
        for (let ci = 0; ci < choices.length; ci++) {
          const letter = String.fromCharCode(65 + ci);
          body += _emitChoiceParagraph(letter, stripChoiceLabel(choices[ci]),
            `<w:pPr><w:keepLines/><w:ind w:left="720"/><w:spacing w:after="80"/></w:pPr>`);
        }

        if (includeAnswers && p?.answer) {
          body += _qAnswerBlock(stripChoiceLabel(p.answer), p.explanation || null);
        }
      });
    ...
    if (_isMCQ(q)) {
      for (let ci = 0; ci < q.choices.length; ci++) {
        ...
        if (isGraphChoice(c)) {
          ...
          body += _emitChoiceParagraph(letter, stripChoiceLabel(c),
            `<w:pPr><w:keepLines/><w:ind w:left="360"/><w:spacing w:after="80"/></w:pPr>`);
        }
      }
    }
