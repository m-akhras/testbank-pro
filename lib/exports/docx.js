...
function buildCoverPageXml({
  logoRid, universityName, courseName, courseCode, examTitle,
  date, semester, timeAllowed, totalPoints, instructions, instructorName,
}) {
  let xml = "";
  if (logoRid) {
    // ~200px wide × ~140px tall at 9525 EMU/px
    xml += `<w:p><w:pPr><w:jc w:val="center"/><w:spacing w:after="200"/></w:pPr>${logoImageXml(logoRid, 1905000, 1333500)}</w:p>`;
  }
  if (universityName) xml += centerPara(universityName, { bold: true, size: 48, spacing: 120 });
  const courseLine = [courseName, courseCode].filter(Boolean).join(" — ");
  if (courseLine) xml += centerPara(courseLine, { size: 32, color: "444444", spacing: 600 });
  if (examTitle) xml += centerPara(examTitle, { bold: true, size: 56, spacing: 200 });
  const dateLine = [date, semester].filter(Boolean).join(" · ");
  if (dateLine) xml += centerPara(dateLine, { size: 24, color: "555555", spacing: 120 });
  const metaLine = [
    timeAllowed && `Time allowed: ${timeAllowed}`,
    totalPoints && `Total: ${totalPoints} points`,
  ].filter(Boolean).join("   ·   ");
  if (metaLine) xml += centerPara(metaLine, { size: 24, color: "555555", spacing: 600 });
  if (instructorName) xml += centerPara(`Instructor: ${instructorName}`, { size: 22, color: "666666", spacing: 400 });
  xml += `<w:p><w:pPr><w:spacing w:after="200"/></w:pPr><w:r><w:rPr><w:sz w:val="24"/></w:rPr><w:t xml:space="preserve">Name: _______________________________________________</w:t></w:r></w:p>`;
  xml += `<w:p><w:pPr><w:spacing w:after="400"/></w:pPr><w:r><w:rPr><w:sz w:val="24"/></w:rPr><w:t xml:space="preserve">Student ID: ___________________________</w:t></w:r></w:p>`;
  if (instructions) {
    xml += `<w:p><w:pPr><w:spacing w:after="80"/></w:pPr><w:r><w:rPr><w:b/><w:sz w:val="24"/></w:rPr><w:t xml:space="preserve">Instructions:</w:t></w:r></w:p>`;
    xml += `<w:p><w:pPr><w:spacing w:after="200"/></w:pPr><w:r><w:rPr><w:i/><w:sz w:val="22"/></w:rPr><w:t xml:space="preserve">${escXml(instructions)}</w:t></w:r></w:p>`;
  }
  xml += `<w:p><w:r><w:br w:type="page"/></w:r></w:p>`;
  return xml;
}

function buildHeaderXml({ logoRid, examTitle, courseCode }) {
  const titleLine = [examTitle, courseCode].filter(Boolean).join(" · ");
  const logoCell = logoRid
    ? `<w:tc><w:tcPr><w:tcW w:w="2000" w:type="dxa"/><w:tcBorders><w:top w:val="nil"/><w:left w:val="nil"/><w:bottom w:val="nil"/><w:right w:val="nil"/></w:tcBorders></w:tcPr><w:p>${logoImageXml(logoRid, 762000, 381000)}</w:p></w:tc>`
...
function applyTemplateAssets({ tc, course, questionCount, logoB64, zip }) {
  ...
  // Logo image — needs ONE relationship per consumer (cover + header) since each header/document part has its own rels
  // but here we simplify: place the logo bytes once and reuse rids.
  if (logoB64) {
    const imgBytes = Uint8Array.from(atob(logoB64), c => c.charCodeAt(0));
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">`;
    ...
    const headerXml = buildHeaderXml({
      logoRid: logoB64 ? "rLogoHeader" : null,
      examTitle: tc.examTitle || "",
      courseCode,
    });
  ...
  let coverXml = "";
  if (includeCover) {
    const totalPoints = (Number(tc.pointsPerQuestion) || 1) * questionCount;
    coverXml = buildCoverPageXml({
      logoRid: logoB64 ? "rLogoCover" : null,
      ...
      instructorName: tc.instructorName || "",
    });
    if (logoB64) {
...
export async function buildDocx(questions, course, vLabel, classSection=null, startNum=1, templateConfig=null) {
  ...
  const logoB64 = tc?.universityLogoUrl && (tc.includeCover || tc.includeHeader)
...
__SECTPR_REFS__
  ...
  __tmplAssets = tc ? applyTemplateAssets({ tc, course, questionCount: questions.length, logoB64, zip }) : null;
...
__SECTPR_REFS__
  ...
  const __tmplAssetsC = tc ? applyTemplateAssets({ tc, course, questionCount: numQ, logoB64, zip }) : null;
