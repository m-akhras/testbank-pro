...
function buildCoverPageXml({
  logoRid, logoCx, logoCy, universityName, courseName, courseCode, examTitle,
  date, semester, timeAllowed, totalPoints, instructions, instructorName,
}) {
  let xml = "";
  if (logoRid) {
    const cx = logoCx || 1905000;
    const cy = logoCy || 1333500; // legacy fallback (~1.4:1 to preserve prior visual)
    xml += `<w:p><w:pPr><w:jc w:val="center"/><w:spacing w:after="200"/></w:pPr>${logoImageXml(logoRid, cx, cy)}</w:p>`;
  }
  ...
  // Page break is emitted by the caller as part of the section-break paragraph
  return xml;
}
