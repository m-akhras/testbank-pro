...
// Cover info table — 9-col base grid (1000 each), rows use gridSpan to merge.
// Total table width 9000 twips. All cells share single 4sz #444444 borders.
function buildInfoTable({
  ...
  // Cell builder with gridSpan support.
  // opts: gridSpan (default 1), bold, italic, label (sets bold+222222), centered, valign
  const mkCell = (text, opts = {}) => {
    const { gridSpan = 1, label = false, bold = label, italic = false, centered = false, valign = "center" } = opts;
    const span = gridSpan > 1 ? `<w:gridSpan w:val="${gridSpan}"/>` : "";
    const width = gridSpan * 1000;
  ...
  // Tall row (~1800 twips minimum) with extra vertical padding.
  ...
  // Row 1 — Semester (1) | <semester> (8)
  const row1 = `<w:tr>${mkCell("Semester", { gridSpan: 1, label: true })}${mkCell(semester || "", { gridSpan: 8 })}</w:tr>`;

  // Row 2 — Exam Date (1) | date (2) | Exam Time: (1) | time (2) | Exam Duration: (1) | duration (2)
  const row2 = `<w:tr>${mkCell("Exam Date", { gridSpan: 1, label: true })}${mkCell(date || "", { gridSpan: 2 })}${mkCell("Exam Time:", { gridSpan: 1, label: true })}${mkCell(time || "", { gridSpan: 2 })}${mkCell("Exam Duration:", { gridSpan: 1, label: true })}${mkCell(duration || "", { gridSpan: 2 })}</w:tr>`;

  // Row 3 — Course Title (1) | <courseTitle> (5) | Course Code (1) | <courseCode> (2)
  const row3 = `<w:tr>${mkCell("Course Title", { gridSpan: 1, label: true })}${mkCell(courseTitle || "", { gridSpan: 5 })}${mkCell("Course Code", { gridSpan: 1, label: true })}${mkCell(courseCode || "", { gridSpan: 2 })}</w:tr>`;

  // Row 4 — Instructor (1) | <instructor> (8)
  const row4 = `<w:tr>${mkCell("Instructor", { gridSpan: 1, label: true })}${mkCell(instructor || "", { gridSpan: 8 })}</w:tr>`;

  // Row 5 — Name (1) | <blank> (4) | ID No. (1) | <blank> (2) | Sect.: NN combined (1)
  const row5 = `<w:tr>${mkCell("Name", { gridSpan: 1, label: true })}${mkCell("", { gridSpan: 4 })}${mkCell("ID No.", { gridSpan: 1, label: true })}${mkCell("", { gridSpan: 2 })}${mkCell(sectCombined || "Sect.: ", { gridSpan: 1, bold: true, centered: true })}</w:tr>`;

  // Row 6 — Materials Allowed (full width, multi-paragraph, tall)
  ...
  return `<w:tbl>
    ...
    <w:tblGrid>
      <w:gridCol w:w="1000"/><w:gridCol w:w="1000"/><w:gridCol w:w="1000"/><w:gridCol w:w="1000"/><w:gridCol w:w="1000"/><w:gridCol w:w="1000"/><w:gridCol w:w="1000"/><w:gridCol w:w="1000"/><w:gridCol w:w="1000"/>
    </w:tblGrid>
    ${row1}
    ${row2}
    ${row3}
...
function buildCoverPageXml({
  ...
  // 1. Logo (centered) — natural aspect ratio when known. Width ~3.65" (3500000 EMU)
  if (logoRid) {
    const cx = logoCx || 3500000;
    const cy = logoCy || 2450000; // 1.43:1 fallback when natural dims unavailable
    xml += `<w:p><w:pPr><w:jc w:val="center"/><w:spacing w:after="120"/></w:pPr>${logoImageXml(logoRid, cx, cy)}</w:p>`;
  }

  // 2. Blue title banner — flush against the info table (no spacer paragraph between)
  ...
  const sectCombined = `Sect.: ${showSection ? (sectionLabel || "") : ""}`;
  xml += buildInfoTable({
...
function applyTemplateAssets({ tc, course, classSection = null, questionCount, logoData, zip }) {
  ...
  if (logoData?.base64) {
    ...
    if (logoData.width && logoData.height) {
      ...
      coverLogoCx = 3500000;
      coverLogoCy = Math.round(3500000 * ratio);
      headerLogoCx = 762000;
...
export async function buildDocx(questions, course, vLabel, classSection=null, startNum=1, templateConfig=null) {
  ...
  function buildQuestionHeader(num, text, marks) {
    ...
    const ppr = `<w:pPr><w:tabs><w:tab w:val="right" w:pos="9000"/></w:tabs><w:spacing w:after="120"/></w:pPr>`;
