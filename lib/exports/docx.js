...
function buildSpacerBlock(heightTwips = 2400) {
  return `<w:tbl>
    <w:tblPr>
      <w:tblW w:w="9000" w:type="dxa"/>
      <w:tblBorders>
...
// Cover info table — 6-row mixed grid matching a real university quiz layout.
// Base grid: 6 columns (1500/1900/1300/1500/1300/1500 = 9000 twips); other rows
// span and approximate widths. All cells share single 4sz #444444 borders.
function buildInfoTable({
  semester, date, time, duration,
  courseTitle, courseCode,
  instructor, name, idNo, section,
  materialsAllowed,
}) {
  ...
  // Cell builder. opts: width (dxa), gridSpan?, label (bool), italic (bool), bold (bool), valign
  const mkCell = (text, opts = {}) => {
    const { width, gridSpan, label = false, italic = false, bold = label, valign = "center" } = opts;
  ...
  // Materials Allowed row uses a 2-paragraph cell (bold label + italic body)
  ...
  // Row 1 — Semester (label) | value spanning 5 cols
  const row1 = `<w:tr>${mkCell("Semester", { width: 1500, label: true })}${mkCell(semester || "", { width: 7500, gridSpan: 5 })}</w:tr>`;

  // Row 2 — Exam Date | <date> | Exam Time | <time> | Exam Duration | <duration>
  const row2 = `<w:tr>${mkCell("Exam Date", { width: 1500, label: true })}${mkCell(date || "", { width: 1900 })}${mkCell("Exam Time", { width: 1300, label: true })}${mkCell(time || "", { width: 1500 })}${mkCell("Exam Duration", { width: 1300, label: true })}${mkCell(duration || "", { width: 1500 })}</w:tr>`;

  // Row 3 — Course Title | <courseTitle, span 2> | Course Code | <courseCode, span 2>
  const row3 = `<w:tr>${mkCell("Course Title", { width: 1500, label: true })}${mkCell(courseTitle || "", { width: 3200, gridSpan: 2 })}${mkCell("Course Code", { width: 1500, label: true })}${mkCell(courseCode || "", { width: 2800, gridSpan: 2 })}</w:tr>`;

  // Row 4 — Instructor | <instructor, span 5>
  const row4 = `<w:tr>${mkCell("Instructor", { width: 1500, label: true })}${mkCell(instructor || "", { width: 7500, gridSpan: 5 })}</w:tr>`;

  // Row 5 — Name | <blank, span 2> | ID No. | <blank> | Sect.
  const row5 = `<w:tr>${mkCell("Name", { width: 1500, label: true })}${mkCell(name || "", { width: 3200, gridSpan: 2 })}${mkCell("ID No.", { width: 1500, label: true })}${mkCell(idNo || "", { width: 1300 })}${mkCell(section || "", { width: 1500 })}</w:tr>`;

  // Row 6 — Materials Allowed (full width, multi-paragraph cell)
  const row6 = `<w:tr>${mkMaterialsCell(materialsAllowed)}</w:tr>`;
  ...
  return `<w:tbl>
    ...
    <w:tblGrid>
      <w:gridCol w:w="1500"/><w:gridCol w:w="1900"/><w:gridCol w:w="1300"/><w:gridCol w:w="1500"/><w:gridCol w:w="1300"/><w:gridCol w:w="1500"/>
    </w:tblGrid>
    ${row1}
    ${row2}
    ${row3}
...
function buildGradingTable(questionMarks) {
  ...
  const totalW = 1200;
  const qW = Math.floor((9000 - totalW) / Math.max(n, 1));
...
function buildCoverPageXml({
  ...
  // 3. Info table (Semester / Exam Date+Time+Duration / Course Title+Code / Instructor / Name+ID+Sect / Materials)
  xml += buildInfoTable({
  ...
  // 4. Blank spacer block
  xml += buildSpacerBlock(2400);
...
export async function buildDocx(questions, course, vLabel, classSection=null, startNum=1, templateConfig=null) {
  ...
  function para(text, opts={}) {
    ...
    if (rightText) {
      ...
      const ppr = `<w:pPr><w:tabs><w:tab w:val="right" w:pos="9000"/></w:tabs><w:spacing w:after="${spacing}"/>${indent?`<w:ind w:left="${indent}"/>`:''}</w:pPr>`;
  ...
  for (let i = 0; i < questions.length; i++) {
    ...
    const qMarks = tc?.questionMarks?.[i] ?? null;
    const marksOpt = (tc && qMarks != null)
      ? { rightText: `[${Number(qMarks) || 10} marks]` }
      : (tc ? { rightText: "[10 marks]" } : {});
    if (q.type === "Branched") {
      body += para(`${num}. [${q.section}] — ${q.difficulty}`, {bold:true, size:24, spacing:80, ...marksOpt});
      ...
      body += para(`${num}. [${q.section}] — ${q.type} — ${q.difficulty}`, {bold:true, size:24, spacing:80, ...marksOpt});
