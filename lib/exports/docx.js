...
function buildInfoTable({
  ...
  // Row 1 — spans [1, 8] — Semester (1500) | <semester> (7860)
  const row1 = `<w:tr>${mkCell("Semester", { width: 1500, gridSpan: 1, label: true })}${mkCell(semester || "", { width: 7860, gridSpan: 8 })}</w:tr>`;

  // Row 2 — spans [1, 2, 1, 2, 1, 2] — Date(1500)|val(1600)|Time(1300)|val(1600)|Duration(1500)|val(1860)
  const row2 = `<w:tr>${mkCell("Exam Date", { width: 1500, gridSpan: 1, label: true })}${mkCell(date || "", { width: 1600, gridSpan: 2 })}${mkCell("Exam Time:", { width: 1300, gridSpan: 1, label: true })}${mkCell(time || "", { width: 1600, gridSpan: 2 })}${mkCell("Exam Duration:", { width: 1500, gridSpan: 1, label: true })}${mkCell(duration || "", { width: 1860, gridSpan: 2 })}</w:tr>`;

  // Row 3 — spans [1, 5, 1, 2] — Course Title(1500)|val(4500)|Course Code(1500)|val(1860)
  const row3 = `<w:tr>${mkCell("Course Title", { width: 1500, gridSpan: 1, label: true })}${mkCell(courseTitle || "", { width: 4500, gridSpan: 5 })}${mkCell("Course Code", { width: 1500, gridSpan: 1, label: true })}${mkCell(courseCode || "", { width: 1860, gridSpan: 2 })}</w:tr>`;

  // Row 4 — spans [1, 8] — Instructor(1500)|val(7860)
  const row4 = `<w:tr>${mkCell("Instructor", { width: 1500, gridSpan: 1, label: true })}${mkCell(instructor || "", { width: 7860, gridSpan: 8 })}</w:tr>`;

  // Row 5 — spans [1, 4, 1, 2, 1] — Name(1500)|blank(3800)|ID No.(700)|blank(2200)|Sect.: NN(1160)
  const row5 = `<w:tr>${mkCell("Name", { width: 1500, gridSpan: 1, label: true })}${mkCell("", { width: 3800, gridSpan: 4 })}${mkCell("ID No.", { width: 700, gridSpan: 1, label: true })}${mkCell("", { width: 2200, gridSpan: 2 })}${mkCell(sectCombined || "Sect.: ", { width: 1160, gridSpan: 1, bold: true, centered: true })}</w:tr>`;
  ...
  return `<w:tbl>
    ...
    <w:tblGrid>
      <w:gridCol w:w="1500"/><w:gridCol w:w="900"/><w:gridCol w:w="700"/><w:gridCol w:w="1300"/><w:gridCol w:w="900"/><w:gridCol w:w="700"/><w:gridCol w:w="1500"/><w:gridCol w:w="700"/><w:gridCol w:w="1160"/>
    </w:tblGrid>
    ${row1}
    ${row2}
    ${row3}
...
function buildGradingTable(questionMarks) {
  ...
  const totalW = 1360;
  const qW = Math.floor((9360 - totalW) / Math.max(n, 1));
