...
function buildInfoTable({ date, semester, time, version, section }) {
  const tcMar = `<w:tcMar><w:top w:w="80" w:type="dxa"/><w:bottom w:w="80" w:type="dxa"/><w:left w:w="100" w:type="dxa"/><w:right w:w="100" w:type="dxa"/></w:tcMar>`;
  const labelCell = (text) => `<w:tc>
    <w:tcPr>
    ...
    <w:p>
      ...
      <w:r><w:rPr><w:b/><w:sz w:val="22"/><w:color w:val="333333"/></w:rPr><w:t xml:space="preserve">${escXml(text)}</w:t></w:r>
    </w:p>
  </w:tc>`;
  const valueCell = (text) => `<w:tc>
    <w:tcPr>
      <w:tcW w:w="6600" w:type="dxa"/>
      <w:tcBorders><w:top w:val="nil"/><w:left w:val="nil"/><w:right w:val="nil"/><w:bottom w:val="single" w:sz="4" w:color="888888"/></w:tcBorders>
      ${tcMar}
    </w:tcPr>
    <w:p>
      <w:pPr><w:jc w:val="left"/><w:spacing w:after="0"/></w:pPr>
      <w:r><w:rPr><w:sz w:val="22"/><w:color w:val="000000"/></w:rPr><w:t xml:space="preserve">${escXml(text)}</w:t></w:r>
    </w:p>
  </w:tc>`;
  const row = (label, value) => `<w:tr>${labelCell(label)}${valueCell(value)}</w:tr>`;
...
function buildGradingTable(questionMarks) {
  ...
  // Default any missing/zero entry to 10
  ...
  const tcMar = `<w:tcMar><w:top w:w="80" w:type="dxa"/><w:bottom w:w="80" w:type="dxa"/><w:left w:w="80" w:type="dxa"/><w:right w:w="80" w:type="dxa"/></w:tcMar>`;
  const fullBorder = `<w:tcBorders><w:top w:val="single" w:sz="4" w:color="666666"/><w:left w:val="single" w:sz="4" w:color="666666"/><w:bottom w:val="single" w:sz="4" w:color="666666"/><w:right w:val="single" w:sz="4" w:color="666666"/></w:tcBorders>`;
  // Header row's first cell — clean look (no top/left/right border, only bottom)
  const cleanLabelHeaderBorder = `<w:tcBorders><w:top w:val="nil"/><w:left w:val="nil"/><w:right w:val="nil"/><w:bottom w:val="single" w:sz="4" w:color="666666"/></w:tcBorders>`;

  const cell = (text, opts = {}) => {
    const { width, bold = false, shading, border = fullBorder, valign = "center" } = opts;
    ...
    return `<w:tc>
      ...
      <w:p>
        ...
        <w:r><w:rPr>${bold ? "<w:b/>" : ""}<w:sz w:val="22"/></w:rPr><w:t xml:space="preserve">${escXml(text)}</w:t></w:r>
      </w:p>
    </w:tc>`;
  };

  // Header row
  let headerRow = `<w:tr>` + cell("", { width: labelW, border: cleanLabelHeaderBorder });
  for (let i = 0; i < n; i++) {
    headerRow += cell(`Q${i + 1}`, { width: qW, bold: true, shading: "E8E8E8" });
  }
  headerRow += cell("Total", { width: totalW, bold: true, shading: "E8E8E8" });
  headerRow += `</w:tr>`;

  // "Out of" row
  let outOfRow = `<w:tr>` + cell("Out of", { width: labelW, bold: true, shading: "F8F8F8" });
  for (let i = 0; i < n; i++) {
    outOfRow += cell(String(marks[i]), { width: qW, shading: "F8F8F8" });
  }
  outOfRow += cell(String(total), { width: totalW, bold: true, shading: "F8F8F8" });
  outOfRow += `</w:tr>`;

  // "Score" row — instructor fills by hand
  const scoreHeight = `<w:trPr><w:trHeight w:val="500" w:hRule="atLeast"/></w:trPr>`;
  let scoreRow = `<w:tr>${scoreHeight}` + cell("Score", { width: labelW, bold: true });
  for (let i = 0; i < n; i++) {
    scoreRow += cell("", { width: qW });
  }
  scoreRow += cell("", { width: totalW });
  scoreRow += `</w:tr>`;

  return `<w:tbl>
    <w:tblPr>
      <w:tblW w:w="${labelW + qW * n + totalW}" w:type="dxa"/>
      <w:jc w:val="center"/>
      <w:tblBorders>
...
function buildCoverPageXml({
  ...
  examTitle,
  date, semester, timeAllowed, instructions,
  showVersion, versionLabel, showSection, sectionLabel,
  questionMarks,
}) {
  let xml = "";

  // 1. Logo (centered) — natural aspect ratio when known
  if (logoRid) {
    const cx = logoCx || 1905000;
    const cy = logoCy || 1333500; // legacy fallback (~1.4:1)
    xml += `<w:p><w:pPr><w:jc w:val="center"/><w:spacing w:after="120"/></w:pPr>${logoImageXml(logoRid, cx, cy)}</w:p>`;
  }

  // 2. Exam title (centered, large, bold) — already course-code-stripped by caller
  if (examTitle) {
    xml += centerPara(examTitle, { bold: true, size: 56, spacing: 240 });
  }

  // 3. Info table (Student Name, Student ID, Date, Semester, Time, Version, Section)
  xml += buildInfoTable({
    date: date || "",
    semester: semester || "",
    time: timeAllowed || "",
    version: showVersion ? (versionLabel || "") : "",
    section: showSection ? (sectionLabel || "") : "",
  });

  // 4. Spacer
  xml += `<w:p><w:pPr><w:spacing w:after="240"/></w:pPr></w:p>`;

  // 5. Instructions block (only when present)
  if (instructions) {
    xml += `<w:p><w:pPr><w:spacing w:after="80"/></w:pPr><w:r><w:rPr><w:b/><w:sz w:val="22"/></w:rPr><w:t xml:space="preserve">Instructions:</w:t></w:r></w:p>`;
    xml += `<w:p><w:pPr><w:jc w:val="both"/><w:spacing w:after="240"/></w:pPr><w:r><w:rPr><w:sz w:val="22"/><w:color w:val="222222"/></w:rPr><w:t xml:space="preserve">${escXml(instructions)}</w:t></w:r></w:p>`;
  }

  // 6. Spacer
  xml += `<w:p><w:pPr><w:spacing w:after="360"/></w:pPr></w:p>`;

  // 7. Grading table (centered, label | Q1..Qn | Total — three rows: header, Out of, Score)
...
function buildFooterPageOfPagesXml() {
  const sty = `<w:rPr><w:sz w:val="18"/><w:color w:val="999999"/></w:rPr>`;
  // SECTIONPAGES (not NUMPAGES) so the cover's section is excluded from "of Y"
  return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<w:ftr xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">
  <w:p><w:pPr><w:jc w:val="center"/></w:pPr>
    ...
    <w:r>${sty}<w:fldChar w:fldCharType="begin"/></w:r>
    <w:r>${sty}<w:instrText xml:space="preserve"> SECTIONPAGES </w:instrText></w:r>
    <w:r>${sty}<w:fldChar w:fldCharType="separate"/></w:r>
    <w:r>${sty}<w:t>1</w:t></w:r>
    <w:r>${sty}<w:fldChar w:fldCharType="end"/></w:r>
  </w:p>
...
function applyTemplateAssets({ tc, course, questionCount, logoData, zip }) {
  ...
  if (logoData?.base64) {
    ...
    if (logoData.width && logoData.height) {
      ...
      coverLogoCx = 1905000;
      coverLogoCy = Math.round(1905000 * ratio);
      headerLogoCx = 762000;
...
<w:sectPr>
  <w:pgSz w:w="12240" w:h="15840"/>
  <w:pgMar w:top="${tc?.includeHeader ? 1800 : 1440}" w:right="1440" w:bottom="1440" w:left="1440"/>
...
__SECTPR_REFS__
  ...
  __tmplAssets = tc ? applyTemplateAssets({ tc, course, questionCount: questions.length, logoData, zip }) : null;
...
__SECTPR_REFS__
  ...
  const __tmplAssetsC = tc ? applyTemplateAssets({ tc, course, questionCount: numQ, logoData, zip }) : null;
