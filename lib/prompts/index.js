...
function buildAnchorRulesText({ sectionNum, variantLabels, anchorLabel = "A", courseModule }) {
...
export function buildAllSectionsPrompt(selectedQuestions, labels, numClassSections, course="", versionMutationType={}, courseObject=null, sectionsToEmit=null, anchorLabel="A") {
  ...
  const sectionRules = sections
    .map(s => buildAnchorRulesText({ sectionNum: s, variantLabels: labels, anchorLabel, courseModule: courseModule2 }))
    .join("\n");
...
export function buildSectionPastePrompt({
  ...
  const anchorRules = buildAnchorRulesText({ sectionNum, variantLabels: labs, anchorLabel, courseModule });
