...
MATH NOTATION RULES:
- USE proper math symbols directly in all text fields:
  - Square roots: √x  (not sqrt(x))
  - Cube roots: ∛x  (not cbrt(x))
  - Superscripts: x², x³, x⁴, x⁵, x⁶, x⁷, x⁸, x⁹  (not x^2)
  - For superscripts > 9 or with variables/expressions: keep ^ syntax: x^10, x^(n+1), e^(2x)
  - Subscripts: x₁, x₂, x₃, x_n  (use Unicode subscripts ₀₁₂₃₄₅₆₇₈₉ when possible; use _ for variables)
  - Greek letters: α β γ δ θ λ μ π σ φ ω  (not alpha, beta, theta, pi, sigma)
  - Comparison: ≤ ≥ ≠  (not <=, >=, !=)
  - Multiplication: · or ×  (not *)
  - Infinity: ∞  (not infinity)
  - Plus/minus: ±
  - Approximately: ≈
- Fractions still use (numerator)/(denominator) — e.g. (10)/(s³) — because Unicode fraction rendering varies by font.
- Integrals stay in plain English: "integral from a to b of f(x) dx" — for the same font-rendering reason.
  - Double integral with bounds: "double integral from a to b of integral from c to d of f(x,y) dx dy"
  - Double integral over a region: "double integral over D of f(x,y) dA"
  - Triple integral over a region: "triple integral over E of f(x,y,z) dV"
  - Contour integral: "contour integral of f dz"
- Set theory and logic symbols: ∪ ∩ ⊆ ⊂ ⊇ ⊃ ∈ ∉ ∅ ∀ ∃ ∧ ∨ ~ → ↔ (preferred for Discrete Math).`;
