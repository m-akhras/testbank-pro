...
import { expectedVersionKeys } from "../exams/versionMerge.js";
...
- NEVER use Unicode math symbols. Write them in words:
  ...
  - Comparison: <=, >=, !=, NOT ≤ ≥ ≠
  - Multiplication in DISPLAYED math (question/answer/explanation): JUXTAPOSE — write 3x not 3*x, 2(x+1) not 2*(x+1), xy not x*y, 3ln(x) not 3*ln(x). Use * ONLY between two numeric literals (2*3). NEVER write a redundant unit coefficient (write y, not 1*y; write -y, not -1*y) or a power of one (write y, not y^1). (Graph expression fields fx/fy/fn keep explicit * per the graph rules.)
  - Infinity: infinity, NOT ∞
- LOGARITHMS: "log" means the base-10 logarithm; write "ln" for the natural logarithm (base e). For any other base b, use change-of-base "log(x)/log(b)" (the base cancels, so it is correct regardless of convention). NEVER write bare "log" to mean natural log — that is "ln".
- ONE EXPRESSION = ONE UNIT: when a question contains a mathematical expression, write the ENTIRE expression as a single contiguous unit in plain math syntax — e.g. "Evaluate lim_{x->2} (2x^2 - x + 1)" as ONE expression, NOT split into separate pieces ("lim" then "(2" then "x^2"). Never emit a lone "e", a bare coefficient, or a detached exponent separated from the expression it belongs to. Keep operators, coefficients, parentheses, and the base/exponent together with the rest of the expression so it reads (and exports) as a single equation.
...
// Scope text appended to EVERY function-mutation instruction (the anchor A for
// s ≥ 2, and the function-mutation chunk). Three guards from the live audit:
//   (1) TOPIC SCOPE — the question's `section` field defines the topic; a function
//       mutation must mutate WITHIN topic-bound sections and stay inside the
//       section's own techniques on topic-agnostic ones (no later-chapter ideas).
//   (2) cross-section family sharing is ACCEPTABLE on topic-bound questions —
//       differentiate by structure/numbers, not by forcing an off-topic family.
//   (3) graph-bearing questions must NOT mutate into ln/log (renderer ln is broken).
function buildFunctionMutationScope(baseQuestions, courseModule) {
  const base = Array.isArray(baseQuestions) ? baseQuestions : [];
  const out = [];
  const perQ = base.map((q, i) => {
    const tc = courseModule?.sectionTopicConstraints?.(q.section);
    return tc ? `  Q${i + 1} [${q.section}]: ${tc.rule}` : null;
  }).filter(Boolean);
  if (perQ.length) {
    out.push(`PER-QUESTION TOPIC SCOPE (the section field defines the topic — mutate within it, never off-topic):\n${perQ.join("\n")}`);
    out.push(`It is ACCEPTABLE for different sections' anchors to share a function family on topic-bound questions — differentiate them by structure/numbers, never by forcing an off-topic family.`);
  }
  // WITHIN-SECTION VARIETY (the carpeted-section fix): cross-section variety alone let
  // the model carpet a whole section with one family (S3 all e^x, S4 all ln). Add a
  // per-question variety rule at the granularity each question's topic demands — but
  // strictly WITHIN the topic scope above (never an off-topic family / later technique).
  const tcsAll = base.map(q => courseModule?.sectionTopicConstraints?.(q.section));
  const anyBound = tcsAll.some(t => t && t.bound);
  const anyAgnostic = tcsAll.some(t => !t || !t.bound);
  if (anyBound) {
    out.push(`WITHIN-SECTION VARIETY (topic-BOUND exp/log sections): vary the MEMBER within the exp/log family across this section's questions — assign each a DIFFERENT member (e.g. Q1 e^x, Q2 ln, Q3 2^x, Q4 log_3, Q5 (1/2)^x; inverse-trig where §1.5 allows). Do NOT default every question to ln. The family stays exp/log (the topic); only the member rotates.`);
  }
  if (anyAgnostic) {
    out.push(`WITHIN-SECTION VARIETY (topic-AGNOSTIC sections): assign each question a DIFFERENT function family across this section (polynomial, rational, sqrt, exponential, trig, log). Do NOT cluster on one family — especially do NOT make them all ln.`);
  }
  if (anyBound || anyAgnostic) {
    out.push(`At most about half the section's questions may share a family/member; never all the same. This variety operates WITHIN the topic scope above — diversity never overrides topic scope (no off-topic families, no later-chapter techniques).`);
  }
  if (base.some(q => q.hasGraph && q.graphConfig)) {
    out.push(`For questions WITH graphs: do NOT mutate into logarithmic (ln/log) functions — the graph renderer's ln output is unreliable. Pick a different on-topic family.`);
  }
  out.push(`GLOBAL: every mutated question MUST remain answerable using ONLY the section's own techniques. Never import a later-chapter concept (e.g. limits at x → ±∞ are a later section, not a basic limit section; no derivative-based reasoning in a pre-derivative section).`);
  return "\n" + out.map(l => `- ${l}`).join("\n");
}

// The function-mutation instruction itself. When EVERY base question is topic-BOUND
// (its family IS its topic) the generic "change the FUNCTION FAMILY" lead-in is
// dropped entirely — the per-question scope's "KEEP the class, mutate within" rule
// is the whole instruction. Otherwise the generic change-family lead-in is kept,
// qualified by the per-question scope (which may pin individual questions in-class).
function buildFunctionMutationRule({ baseQuestions, sectionNum, courseModule, contextLabel }) {
  const base = Array.isArray(baseQuestions) ? baseQuestions : [];
  const tcs = base.map(q => courseModule?.sectionTopicConstraints?.(q.section));
  const allBound = base.length > 0 && tcs.every(t => t && t.bound);
  const scope = buildFunctionMutationScope(base, courseModule);
  if (allBound) {
    return `TOPIC-BOUND mutation (${contextLabel}): KEEP each question's function class — its family IS the section topic, so do NOT switch to a different family. Mutate WITHIN the class per the per-question scope below. Keep DIFFICULTY, STRUCTURE, step count, section, and topic identical.${scope}`;
  }
  const sectionRule = courseModule?.buildSectionRules?.(sectionNum, true) ?? "";
  return `FUNCTION mutation (${contextLabel}): change the FUNCTION FAMILY of each base question (e.g. polynomial → exponential / trigonometric / rational / sqrt), EXCEPT where the per-question scope below says to keep the class. Keep each question's DIFFICULTY, STRUCTURE, step count, section, and topic identical. ${sectionRule}${scope}`;
}

// Per-section ANCHOR RULES text — the SINGLE source of the anchor-model wording,
// shared by buildSectionPastePrompt (one section per response) and
// buildAllSectionsPrompt (all sections in one response) so the two prompts can
// never drift from each other or from the handlePaste guard. Section 1's anchor IS
// the master (not output); section s ≥ 2 FIRST generates its own A (a function
// mutation of the master) THEN its variants as numbers mutations of that A.
function buildAnchorRulesText({ sectionNum, variantLabels, anchorLabel = "A", courseModule, baseQuestions = [] }) {
  const labs = Array.isArray(variantLabels) ? variantLabels : [];
  const variantKeys = labs.map(l => `S${sectionNum}_${l}`);
  if (sectionNum === 1) {
    return `- Section 1's version ${anchorLabel} IS the master above (unchanged — do NOT output it). Generate ${variantKeys.join(", ")} as NUMBERS mutations of the master: change ONLY numbers/coefficients, keep the SAME functions, structure, section, and difficulty.`;
  }
  const fnRule = buildFunctionMutationRule({
    baseQuestions, sectionNum, courseModule,
    contextLabel: `S${sectionNum}_${anchorLabel}, the anchor for classroom section ${sectionNum}`,
  });
  return `- Section ${sectionNum}: FIRST generate S${sectionNum}_${anchorLabel}: ${fnRule}\n- THEN generate ${variantKeys.join(", ")} as NUMBERS mutations of the S${sectionNum}_${anchorLabel} you just generated — same function family as your ${anchorLabel}, change only numbers/coefficients. (Every version in this section shares S${sectionNum}_${anchorLabel}'s functions.)`;
}
...
//
// ANCHOR MODEL: `labels` are the VARIANT labels (B, C…) generated every section.
// The output key set is exactly expectedVersionKeys(numClassSections, labels,
// {anchorMode:true}) — S1's anchor A is the master (not output), every section
// s ≥ 2 also emits its own anchor S{s}_A. This MUST stay === the handlePaste guard.
export function buildAllSectionsPrompt(selectedQuestions, labels, numClassSections, course="", versionMutationType={}, courseObject=null, sectionsToEmit=null, anchorLabel="A") {
  const courseContext = buildCourseContext(courseObject);
  const anyGraph = selectedQuestions.some(q => q.hasGraph && q.graphConfig);
  const lines = selectedQuestions.map((q,i) => {
    const orig = q.question;
  ...
  // ANCHOR MODEL key set. For the default (all sections) this is literally the
  // handlePaste guard's expected set — same helper, so they cannot drift. A
  // sectionsToEmit subset mirrors the same per-section rule (s ≥ 2 emits its A).
  const allKeys = (Array.isArray(sectionsToEmit) && sectionsToEmit.length)
    ? sections.flatMap(s => (s >= 2 ? [anchorLabel, ...labels] : labels).map(v => `S${s}_${v}`))
    : expectedVersionKeys(numClassSections, labels, { anchorMode: true, anchorLabel });

  const courseModule2 = getCourse(course);
  const sectionRules = sections
    .map(s => buildAnchorRulesText({ sectionNum: s, variantLabels: labels, anchorLabel, courseModule: courseModule2, baseQuestions: selectedQuestions }))
    .join("\n");

  const exampleKeys = allKeys
    .map(k => `  "${k}": [{type, section, difficulty, question, choices, answer, explanation${graphShape2}}, ...]`)
    .join(",\n");

  const allSectionsPrompt = `TESTBANK_ALL_SECTIONS_REQUEST
Classroom sections: ${numClassSections}
Variant versions per section: ${versionList} (each section also has an anchor ${anchorLabel})
All version keys to generate: ${allKeys.join(", ")}

The MASTER exam (Section 1's version ${anchorLabel}) is the question set below — do NOT output S1_${anchorLabel}, it is the master unchanged. Every other version is mutated from it per the anchor rules.

Master / base questions (in order):
${lines}

ANCHOR RULES BY SECTION:
${sectionRules}
- Across sections: ${courseModule2?.crossSectionRuleShort ?? "each section must use different functions entirely."}
- ALWAYS regenerate a correct answer key for each mutated version.
- Keep same question type, section name, and difficulty.
- For Multiple Choice: all 4 choices must be distinct values; preserve any "None of these"/"All of the above"; keep the same number of choices as the master.
...
Return a JSON object with EXACTLY these keys (${allKeys.join(", ")}):
{
${exampleKeys}
}
Reply with ONLY valid JSON object, no markdown, no explanation.`;
...
export function buildVersionChunkPrompt({
  ...
  const mutRule = mutation === "function"
    ? buildFunctionMutationRule({
        baseQuestions: base, sectionNum, courseModule,
        contextLabel: `this is the ANCHOR version A for classroom section ${sectionNum}`,
      })
...
export function buildSectionPastePrompt({
  ...
  // Shared anchor-model wording — same source as buildAllSectionsPrompt.
  const anchorRules = buildAnchorRulesText({ sectionNum, variantLabels: labs, anchorLabel, courseModule, baseQuestions: base });
...
export function buildReplacePrompt(q, mutationType="numbers", reason="") {
  // Topic-aware function mutation (parity with the version/section path, 3d3b289):
  // the section field defines the topic — bound sections (§1.4/§1.5) stay in their
  // function class, agnostic limit sections (§2.2/2.3/2.5) may change family but
  // stay within their own techniques, and ln is blocked for graph-bearing questions.
  const replaceCourseModule = getCourse(q.course);
  const functionMutationRule = (() => {
    const tc = replaceCourseModule?.sectionTopicConstraints?.(q.section);
    const lead = tc?.bound
      ? "TOPIC-BOUND: KEEP the question's function class — its family IS the section topic, so do NOT switch to a different family. Mutate WITHIN the class per the per-question scope below. Same concept area, difficulty, and step count."
      : "Use a DIFFERENT function type (e.g. if original uses polynomial, use exponential or trigonometric), EXCEPT where the per-question scope below says to keep the class. Same concept area, same difficulty, same steps count.";
    return lead + buildFunctionMutationScope([q], replaceCourseModule);
  })();
