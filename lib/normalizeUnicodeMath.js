export function normalizeUnicodeMath(s) {
  ...
  // Subscript Unicode → underscore. Single glyph → _X, contiguous run → _{XYZ}.
  // Parallels the superscript pass above. The auto-brace pass below only matches
  // 2+ LETTERS, so digit subscripts (_0, _{01}) are never touched by it.
  r = r.replace(/[₀₁₂₃₄₅₆₇₈₉]+/g, (run) => {
    const map = { '₀':'0','₁':'1','₂':'2','₃':'3','₄':'4','₅':'5','₆':'6','₇':'7','₈':'8','₉':'9' };
    const ascii = run.replace(/./g, (c) => map[c] ?? c);
