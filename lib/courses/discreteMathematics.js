...
SET NOTATION (always use symbols in question text, choices, stems, and explanations — never spell out):
- Union: A ∪ B  (NEVER "A union B")
- Intersection: A ∩ B  (NEVER "A intersect B" or "A intersection B")
- Complement: A'  (NEVER "complement of A" inside an expression)
- Difference: A - B
- Subset: A ⊆ B  (NEVER "A subset B")
- Proper subset: A ⊂ B
- Element of: x ∈ A  (NEVER "x in A" inside an expression)
- Not element of: x ∉ A
- Empty set: ∅
- Cardinality: |A|
- Power set: P(A)
- Set builder: {x ∈ U | P(x)}
These rules apply to all questions in Ch.6 (Set Theory), Ch.7 (Functions), and any Ch.9 question that involves set operations.
