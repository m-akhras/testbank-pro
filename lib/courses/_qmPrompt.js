...
9. SECTION 14.1 ONLY (Simple Linear Regression Model) — content scope rules:
   REQUIRED: every 14.1 question MUST be wrapped in a real-world business scenario with named variables. Question text refers to the scenario, NEVER to abstract "x" and "y".
     Scenario examples (rotate, do not reuse the same one): Promotions vs Weekly Revenue, Advertising Spend vs Sales, Years of Experience vs Salary, Hours Studied vs Exam Score, Number of Stores vs Total Sales, Square Footage vs Monthly Rent, Customer Visits vs Daily Profit, Temperature vs Ice Cream Sales.
     Phrasing example — RIGHT: "A retailer wants to predict weekly revenue (in \$1000s) from the number of in-store promotions. The fitted regression equation is ŷ = 10.2 + 1.8x. Estimate the weekly revenue when 4 promotions are run."
     Phrasing example — WRONG: "Predict y when x = 4 given ŷ = 10.2 + 1.8x."
   ALLOWED 14.1 question types — vary across the question set:
     a. Interpret the slope (b₁) in scenario context — what does it mean per one-unit change in the predictor.
     b. Interpret the y-intercept (b₀) in scenario context — what does it represent when the predictor is 0 (and whether that interpretation is meaningful).
     c. Predict ŷ for a given value of x using the regression equation.
     d. Identify the regression equation from a graph or from summary statistics (Σx, Σy, Σxy, Σx², n).
     e. Determine the direction (positive / negative) and strength (weak / moderate / strong) of the linear relationship from a scatter plot or correlation.
     f. Compute b₀ and b₁ from raw data using b₁ = Σ(xᵢ − x̄)(yᵢ − ȳ) / Σ(xᵢ − x̄)² and b₀ = ȳ − b₁·x̄.
   FORBIDDEN in 14.1 — these belong in later sections, do NOT generate them here:
     - Residual calculations. The word "residual" must NOT appear anywhere in the question text, choices, or explanation.
