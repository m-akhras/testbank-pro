...
// Compose the three textbook fields into the single string the template
// system expects (e.g. "Stewart Early Transcendentals 9th Ed"). Built-in
// courses without textbook info produce an empty string; the admin
// template-generator surfaces this so the user can override.
function composeTextbook(name, author, edition) {
  if (!name && !author && !edition) return "";
  const author_last = (author || "").split(" ").pop(); // "James Stewart" → "Stewart"
  const parts = [];
  if (author_last) parts.push(author_last);
  if (name) parts.push(name);
  if (edition) parts.push(`${edition}th Ed`);
  return parts.join(" ");
}
...
export const COURSES = Object.fromEntries(
  Object.entries(courseModules).map(([name, mod]) => [
    name,
    {
      ...
      textbook_name: mod.textbook_name || "",
      textbook_author: mod.textbook_author || "",
      textbook_edition: mod.textbook_edition || "",
      textbook: composeTextbook(mod.textbook_name, mod.textbook_author, mod.textbook_edition),
    },
  ])
);
