...
// TOPIC-AWARE function-mutation scope (live audit, Calc 1). A "function mutation"
// must respect the topic the question's `section` field names — the audit caught a
// §1.4 Exponential question mutated into quadratics (the family IS the topic) and a
// §2.2 limit mutated into "lim x→∞ e^(2x)" (limits at infinity = §2.6, out of scope).
// Returns null for sections with no special constraint (generic change-the-family
// behavior), or { bound, num, rule }:
//   bound:true  → the function CLASS is the topic itself: KEEP it, mutate WITHIN it.
//   bound:false → topic-AGNOSTIC (any family on-topic) but EXCLUDE later-section ideas.
// Section numbers here are Calc-1-specific (1.4, 1.5, 2.2, 2.3, 2.5); other calc
// courses' sections (3.9, 6.x, 11.x, 14.x, …) don't collide and return null.
export function sectionTopicConstraints(section) {
  const s = String(section || "").trim();
  const num = (s.match(/\d+\.\d+/) || [])[0] || "";
  // TOPIC-BOUND: the function family IS the section's topic — never leave it.
  const BOUND = {
    "1.4": "KEEP the exponential/logarithmic family — it IS this section's topic (Exponential Functions). Mutate WITHIN it: change the base (2^x, 3^x, e^x, (1/2)^x), the exponent structure (x, 2x, -x, x+1), or apply shifts/reflections. Do NOT switch to polynomial, rational, trig, or sqrt — that leaves the topic.",
    "1.5": "KEEP the inverse-function / logarithm / exponential family — it IS this section's topic (Inverse Functions and Logarithms). Mutate WITHIN it: change the base, argument, or composition (ln, log_b, e^x and their inverses). Do NOT switch to polynomial, rational, or trig families.",
  };
  if (BOUND[num]) return { bound: true, num, rule: BOUND[num] };
  // TOPIC-AGNOSTIC: any family is fine, but stay inside the section's own techniques.
  const AGNOSTIC = {
    "2.2": "NO limits at x → ±∞ (those are §2.6 Limits at Infinity), and NO derivative-based reasoning (§2.8+) — evaluate the limit directly.",
    "2.3": "NO limits at x → ±∞ (§2.6) and NO l'Hôpital / derivative shortcuts (§2.8+, §4.4) — use only the limit laws.",
    "2.5": "NO limits at x → ±∞ (§2.6) and NO derivative-based reasoning (§2.8+) — reason only from continuity and one/two-sided limits.",
  };
  if (AGNOSTIC[num]) {
    return { bound: false, num, rule: `Any function family is on-topic — you MAY change the family. STAY within §${num}'s techniques: ${AGNOSTIC[num]}` };
  }
  return null;
}
