...
export function mathToHTMLInline(s) {
  ...
  // exp(arg) → e^arg so it renders with proper superscript via the existing exponent regex
  r = r.replace(/\bexp\(([^)]+)\)/g, "e^($1)");
  ...
  // subscripts (mirror of the exponent block above): b_0, b_{0}, x_(n) → <sub>
  r = r.replace(/([a-zA-Z0-9])_\{([^}]+)\}/g, (_, b, s) => `${b}<sub>${s}</sub>`);
  r = r.replace(/([a-zA-Z0-9])_\(([^)]+)\)/g, (_, b, s) => `${b}<sub>${s}</sub>`);
  r = r.replace(/([a-zA-Z0-9])_(-?[0-9]+)/g,  (_, b, s) => `${b}<sub>${s}</sub>`);
