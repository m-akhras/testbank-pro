...
export default function LoginPage() {
  ...
  async function handleLogin(e) {
    ...
    try {
      ...
      if (signInError) {
        ...
        if (signUpError) { setError(signInError.message); setLoading(false); return; }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0a0a1a", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Inter', sans-serif" }}>
        ...
        <form onSubmit={handleLogin}>
          <div style={{ marginBottom: "1rem" }}>
            ...
            <input type="email" value={email} onChange={e => setEmail(e.target.value)} required placeholder="your@email.com"
              style={{ width: "100%", padding: "0.65rem 0.85rem", background: "#1a1a2e", border: "1px solid #334155", borderRadius: "6px", color: "#e8e8e0", fontSize: "0.9rem", outline: "none", boxSizing: "border-box" }} />
          </div>
          ...
          <div style={{ textAlign: "right", marginBottom: "1.25rem" }}>
            <a href="/reset-password" style={{ fontSize: "0.78rem", color: "#475569", textDecoration: "none" }}>
              Forgot password?
            </a>
          </div>
          {error && (
            <div style={{ background: "#1a0a0a", border: "1px solid #7f1d1d", borderRadius: "6px", padding: "0.65rem 0.85rem", color: "#f87171", fontSize: "0.8rem", marginBottom: "1rem" }}>
              {error}
            </div>
          )}
          <button type="submit" disabled={loading}
            style={{ width: "100%", padding: "0.75rem", background: loading ? "#064e3b" : "#10b981", color: "#fff", border: "none", borderRadius: "6px", fontSize: "0.95rem", fontWeight: "600", cursor: loading ? "not-allowed" : "pointer" }}>
